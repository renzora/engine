//! Update binary download with progress tracking
...
pub fn start_download(state: &mut UpdateState, url: &str, version: &str) {
    ...
    let download_path = updates_dir.join(format!("renzora_editor_{}.exe", version));
...
pub fn launch_updater(new_exe_path: &PathBuf) -> Result<(), String> {
    // Get the current executable path
    let current_exe = std::env::current_exe()
        .map_err(|e| format!("Could not determine current exe path: {}", e))?;

    // Get the updater path (should be in the same directory as the editor)
    let updater_path = current_exe
        .parent()
        .ok_or("Could not determine editor directory")?
        .join("renzora_updater.exe");
    ...
    // Launch updater with arguments: <new_exe_path> <target_exe_path> <editor_pid>
