...
import { IconPhoto, IconWaveSquare, IconFileText, IconFile, IconBox, IconVideo, IconCode, IconFolderOpen, IconFolder, IconCirclePlus, IconCircle, IconRectangle, IconGrid3x3, IconBulb, IconUpload, IconChevronDown, IconChevronRight, IconX, IconCheck, IconSearch, IconMenu2 } from '@tabler/icons-solidjs';
...
import { createContextMenuActions } from '../actions/ContextMenuActions';
import ContextMenu from '../ui/ContextMenu';
import ScriptCreationDialog from '../ui/ScriptCreationDialog';
...
function AssetLibrary({ onContextMenu }) {
  ...
  const getExtensionStyle = (extension) => {
    ...
    if (['.mp3', '.wav', '.ogg', '.m4a'].includes(ext)) {
      return {
        icon: <IconWaveSquare class="w-3 h-3" />,
      ...
      <div class={`${size} bg-gray-700 rounded overflow-hidden transition-all group-hover:scale-110 relative`}>
        <Show when={!error()} fallback={
          <div class="w-full h-full flex items-center justify-center">
            <IconBox class="w-8 h-8 text-purple-500" />
          </div>
        }>
    ...
    if (!thumbnailUrl) {
      return (
        <div class={`${size} bg-gray-700 rounded flex items-center justify-center transition-all group-hover:scale-110`}>
          <IconPhoto class="w-6 h-6 text-green-400" />
        </div>
      );
    }
    ...
    const contextMenuItems = [
      {
        label: 'Create Object',
        action: () => {},
        icon: <IconCirclePlus class="w-4 h-4" />,
        submenu: [
      ...
      { separator: true },
      {
        ...
        icon: <IconFileText class="w-4 h-4" />
      },
      { separator: true },
      {
        label: 'Upload Files...',
        action: () => handleUploadClick(),
        icon: <IconUpload class="w-4 h-4" />,
        shortcut: 'Ctrl+U'
      },
      {
        label: 'Upload Folder...',
        action: () => handleUploadFolderClick(),
        icon: <IconFolderOpen class="w-4 h-4" />
      },
      { separator: true },
      {
        label: 'New Folder',
        action: () => handleCreateFolder(),
        icon: <IconFolder class="w-4 h-4" />
      }
    ];
            ...
            <Show when={is3DModelFile(asset.extension)} fallback={
              <Show when={isImageFile(asset.extension)} fallback={
                <div class={`w-full h-full bg-gray-700 rounded flex items-center justify-center transition-all group-hover:scale-105 ${
                  ...
                  {isScriptFile(asset.extension) ? (
                    <IconCode class="w-8 h-8 text-blue-400" />
                  ) : (
        ...
        <div class="px-2 py-2 border-b border-slate-700">
          ...
          <div class="relative">
            <Show when={isSearching()} fallback={
              <IconSearch class="w-3 h-3 absolute left-2 top-1.5 text-gray-400" />
            }>
                  ...
                  {(crumb, index) => (
                    <>
                      ...
                      <Show when={index() < breadcrumbs().length - 1}>
                        <IconChevronRight class="w-3 h-3 mx-1 text-gray-600" />
                      </Show>
                    </>
                  )}
                </For>
            ...
            <div class="absolute inset-0 flex items-center justify-center bg-blue-900/20 backdrop-blur-sm z-10">
              <div class="text-center">
                <div class="w-16 h-16 mx-auto mb-4 border-2 border-blue-400 border-dashed rounded-lg flex items-center justify-center">
                  <IconUpload class="w-8 h-8 text-blue-400" />
                </div>
              ...
              <div class="flex-1 flex items-center justify-center">
                <Show when={searchQuery()} fallback={
                  ...
                  <div class="w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4 sm:mb-6 border-2 border-gray-600 border-dashed rounded-xl flex items-center justify-center bg-gray-800/30">
                    {(() => {
                      if (viewMode() === 'folder') {
                        return <IconFolderOpen class="w-8 h-8 sm:w-10 sm:h-10 text-gray-500" />;
                      } else {
