...
import ViewportTabs from '@/plugins/editor/ui/ViewportTabs';
import RenderPlugin from '@/plugins/core/render';
import NodeEditor from '@/plugins/editor/viewports/nodeEditor';
import { viewportTypes } from "@/plugins/core/engine";
          ...
          <div className="absolute inset-0 bg-gray-900 flex flex-col">
            <div className="flex items-center justify-between p-3 border-b border-gray-700 bg-gray-800">
              <div className="flex items-center gap-2">
                <IconSettings class="w-4 h-4 text-gray-400" />
        ...
        if (pluginViewportType && pluginViewportType.component) {
          ...
          return (
            <div className="absolute inset-0 bg-gray-900 flex flex-col">
              <div className="flex items-center justify-between p-3 border-b border-gray-700 bg-gray-800">
                <div className="flex items-center gap-2">
                  <pluginViewportType.icon class="w-4 h-4 text-gray-400" />
                  <span className="text-sm font-medium text-white">{tab.name}</span>
                </div>
                <button
                  onClick={() => {
                    ...
                    if (threeDTab) {
                      editorActions.setActiveViewportTab(threeDTab.id);
                    }
                  }}
                  className="p-1 hover:bg-gray-700 rounded transition-colors"
                  title="Close overlay (return to 3D view)"
