...
export function useCameraController(renderer, canvas) {
  ...
  const handleMouseMove = (event) => {
    ...
    if (isLeftMouseDown) {
      ...
      renderer().moveCamera('forward', moveDistance);
    } else if (isRightMouseDown) {
      // Look around (like Babylon.js right mouse)
      const rotateX = deltaY * rotationSpeed() * 180 / Math.PI * precisionMultiplier;
      const rotateY = deltaX * rotationSpeed() * 180 / Math.PI * precisionMultiplier;
      renderer().rotateCamera(rotateY, rotateX);
  ...
  const handleKeyboardMovement = () => {
    ...
    if (keys.w) renderer().moveCamera('forward', finalSpeed);
    if (keys.s) renderer().moveCamera('backward', finalSpeed);
    if (keys.d) renderer().moveCamera('right', finalSpeed);
    if (keys.a) renderer().moveCamera('left', finalSpeed);
    if (keys.e) renderer().moveCamera('up', finalSpeed);
    if (keys.q) renderer().moveCamera('down', finalSpeed);
  };
