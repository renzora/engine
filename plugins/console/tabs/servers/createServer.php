    ...
    start: function() {
        document.getElementById('create_server_btn').addEventListener('click', function() {
            ...
            ui.ajax({
                outputType: 'json',
                method: 'POST',
                url: 'plugins/console/tabs/servers/ajax/createServer.php',
                data: JSON.stringify({ name: serverName }),
                headers: {
                    'Content-Type': 'application/json'
                },
                success: function(data) {
                    console.log('Response from createServer.php:', data); // Log the entire response

                    // Check if the server data is available
                    if (data.message === 'success' && data.server && data.server.id) {
                        plugin.close('server_create_window');
                        ui_servers_tab_window.addServerToList(data.server); // Pass the server object
                        ui_servers_tab_window.loadCreateSceneplugin(data.server.id); // Use server.id
                    } else {
                        console.error('Error creating server:', data.message, data.error);
