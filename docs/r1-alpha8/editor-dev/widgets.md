...
**Author the panel opening downward** (`top: Val::Percent(100.0)`) and let ember place it. It flips to `bottom: 100%` on its own when below is where the room isn't, and it re-decides on every frame the popup is open — so a trigger that moves, or a panel whose height changes with its contents, stays on screen without you tracking it. Hard-coding an upward inset defeats that in the one direction it matters: the panel then opens up even when there is a whole window free below it. (`popup_position` did nothing at all until r1-alpha8 — it queried `GlobalTransform`, which Bevy 0.19's `Node` doesn't have — so anything written against its *absence*, i.e. authored upward on purpose, is now placed dynamically and should be re-checked.)
...
"Out of sight" is measured against the **clip rect the trigger is in**, not the window — for both `dropdown` and `Popup`, which share the same `flips_up` rule. A menu still gets cut off at the bounds of any clipping ancestor, since `GlobalZIndex` fixes paint order and not clipping. So a dropdown at the bottom of a panel's scroll area opens upward even when the window below it is empty (a box near the end of Settings), and the bottom panel's set menu opens down into the panel when the panel is tall and up over the workspace when it's short — the dock wrapper clips it at the status bar either way. Cramped in both directions it stays down, rather than flipping into *less* room than it started with.
...
**A full-screen modal wants `ModalSurface`, not `FocusPolicy::Block`.** The two look interchangeable and are not: `FocusPolicy` settles bevy_ui picking *between UI nodes*, which leaves every system that reads the mouse directly still acting on a pointer that is visually over your panel. The viewport keeps orbiting, picking keeps selecting, panels keep scrolling. `ModalSurface` is what the editor's own routing asks about: `correct_pointer_state` clears `Interaction` and `cursor_over` on everything the modal covers (your panel and its descendants excepted), `scroll_wheel` confines the wheel to scroll areas inside it, and `PointerOverOverlay` goes true for handlers with no UI rect to test. Every dialog `overlay()` builds already carries it.

Its sibling `Overlay` is a separate opt-in and means "a dismissable dialog root": `overlay_dismiss` despawns it on Escape, the ✕, or a click outside an `OverlayCard`. Add it only if you want that behaviour. A modal that owns its own lifecycle (the Settings overlay, the splash overlay) takes `ModalSurface` alone, or the despawn fights whatever spawns it and the "outside" test fires on every click inside a panel that is not an `OverlayCard`.
