...
- ⬜ **Host-server mode** (Lightyear `HostClient`) — one process = server + a local client (no UDP for the local player). FEASIBLE on 0.26.4: host-ness is tracked by the `HostClient`/`HostServer` marker components, and the `HostPlugin` observers + all the prediction/replication/netcode gating on `With<HostClient>` are fully implemented. (The unused `is_host_server()` `todo!()` is dead code — never called — so it does NOT block this.) Remaining work is engine-side, not a library defect: (a) add both `ClientPlugins` + `ServerPlugins` to a host build (lightyear explicitly supports this — `client` + `server` are both default features), (b) register the protocol exactly once, (c) start the server + spawn the host client as `(Client, LinkOf { server }, …)` and trigger `Connect` — the observers promote it to `HostClient`. The exact host-client link/IO components are undocumented (infer from `lightyear_connection::host`), and it's untestable locally (Docker rebuild to verify).
