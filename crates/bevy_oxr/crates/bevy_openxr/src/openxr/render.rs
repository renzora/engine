...
pub fn init_views<const SPAWN_CAMERAS: bool>(
    graphics_info: Option<Res<OxrCurrentSessionConfig>>,
    mut manual_texture_views: ResMut<ManualTextureViews>,
    swapchain_images: Option<Res<OxrSwapchainImages>>,
    mut commands: Commands,
) {
    let (Some(graphics_info), Some(swapchain_images)) = (graphics_info, swapchain_images) else {
        warn!("init_views: session resources not yet available, skipping");
        return;
    };
...
pub fn wait_frame(mut frame_waiter: ResMut<OxrFrameWaiter>, mut commands: Commands) {
    match frame_waiter.wait() {
        ...
        Err(e) => {
            warn!("XR wait_frame failed (session may have ended): {e}");
        }
    }
}
...
pub fn insert_texture_views(
    ...
    frame_state: Res<OxrFrameState>,
    mut active: ResMut<OxrFrameActive>,
) {
    if !active.0 || !frame_state.should_render {
        return;
    }
    let index = match swapchain.acquire_image() {
        Ok(i) => i,
        Err(e) => {
            warn!("XR acquire_image failed (session may have ended): {e}");
            active.0 = false;
            return;
        }
    };
...
pub fn release_image(mut swapchain: ResMut<OxrSwapchain>, state: Res<OxrFrameState>, active: Res<OxrFrameActive>) {
    ...
    if let Err(e) = swapchain.release_image() {
        warn!("XR release_image failed (session may have ended): {e}");
    }
}

pub fn end_frame(world: &mut World) {
    // Skip end_frame if begin_frame didn't succeed — calling end without begin
    // violates the OpenXR spec and can cause flickering or runtime errors.
    if !world.resource::<OxrFrameActive>().0 {
        return;
    }
    #[cfg(target_os = "android")]
    {
