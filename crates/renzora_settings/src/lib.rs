...
fn draw_settings_overlay(world: &mut World, ctx: &egui::Context) {
    ...
    let settings = world
        ...
        .cloned()
        .unwrap_or_default();
    let custom_fonts = world
        .get_resource::<CustomFonts>()
        .cloned()
        .unwrap_or_default();
    let keybindings = world
    ...
    let input_map = world
        .get_resource::<renzora_input::InputMap>()
        .cloned()
        .unwrap_or_default();
    ...
    if input_ui_state.listening {
        ...
        for gamepad in gp_query.iter(world) {
            ...
            if input_ui_state.captured_gamepad.is_some() {
                break;
            }
            ...
            if left.length() > 0.8 {
                input_ui_state.captured_gamepad = Some(renzora_input::InputBinding::GamepadAxis(
                    "LeftStickX".into(),
                ));
                break;
            }
    ...
    egui::Window::new("Settings")
        ...
        .show(ctx, |ui| {
            ...
            let header_resp = egui::Frame::new()
                ...
                .response;
            let header_hover = ui.interact(
                header_resp.rect,
                ui.id().with("settings_header_hover"),
                egui::Sense::hover(),
            );
            ...
            ui.horizontal_top(|ui| {
                ...
                let content_corners = CornerRadius {
                    nw: 6,
                    ne: 0,
                    sw: 0,
                    se: 0,
                            ...
                            egui::ScrollArea::vertical()
                                ...
                                .show(ui, |ui| {
                                    ...
                                    match settings_mut.settings_tab {
                                        SettingsTab::Project => render_project_tab(
                                            ui,
                                            &mut project_config_mut,
                                            &scene_files,
                                            project_path.as_deref(),
                                            &mut clear_thumbnail_cache,
                                            &theme,
                                        ),
                                        SettingsTab::Interface => render_interface_tab(
                                            ui,
                                            &mut settings_mut,
                                            &custom_fonts,
                                            &theme,
                                        ...
                                        SettingsTab::Editor => {
                                            render_editor_tab(ui, &mut settings_mut, &theme)
                                        }
                                        SettingsTab::Viewport => render_viewport_tab(
                                            ui,
                                            &mut settings_mut,
                                            &mut viewport_mut,
                                            &theme,
                                        ...
                                        SettingsTab::Scripting => {
                                            render_scripting_tab(ui, &mut settings_mut, &theme)
                                        }
                                        SettingsTab::Assets => {
                                            render_assets_tab(ui, &mut settings_mut, &theme)
                                        }
                                        SettingsTab::Input => render_input_tab(
                                            ui,
                                            &mut input_map_mut,
                                            &mut input_ui_state,
                                            &theme,
                                        ),
                                        SettingsTab::Shortcuts => render_shortcuts_tab(
                                            ui,
                                            &mut keybindings_mut,
                                            &plugin_shortcuts,
                                            &theme,
                                        ...
                                        SettingsTab::Theme => {
                                            render_theme_tab(ui, &mut theme_edit, &theme)
                                        }
                                        SettingsTab::Plugins => {
                                            render_plugins_tab(ui, &mut settings_mut, &theme)
                                        }
                                    }
                                });
                        });
    ...
    if clear_thumbnail_cache {
        if let Some(ref path) = project_path {
            let thumbs_dir = path.join(".cache").join("thumbnails").join("materials");
            match std::fs::remove_dir_all(&thumbs_dir) {
                Ok(_) => info!(
                    "[settings] Cleared material thumbnail cache at {}",
                    thumbs_dir.display()
                ),
        ...
        if let Some(mut registry) =
            world.get_resource_mut::<renzora_editor::MaterialThumbnailRegistry>()
        {
...
impl CategoryStyle {
    fn interface() -> Self {
        Self {
            accent_color: Color32::from_rgb(99, 178, 238),
            header_bg: Color32::from_rgb(35, 45, 55),
        }
    }
    fn camera() -> Self {
        Self {
            accent_color: Color32::from_rgb(178, 132, 209),
            header_bg: Color32::from_rgb(42, 38, 52),
        }
    }
    fn developer() -> Self {
        Self {
            accent_color: Color32::from_rgb(236, 154, 120),
            header_bg: Color32::from_rgb(50, 40, 38),
        }
    }
    fn grid() -> Self {
        Self {
            accent_color: Color32::from_rgb(134, 188, 126),
            header_bg: Color32::from_rgb(35, 48, 42),
        }
    }
    fn gizmos() -> Self {
        Self {
            accent_color: Color32::from_rgb(120, 200, 200),
            header_bg: Color32::from_rgb(35, 48, 50),
        }
    }
    fn shortcuts() -> Self {
        Self {
            accent_color: Color32::from_rgb(247, 207, 100),
            header_bg: Color32::from_rgb(50, 45, 35),
        }
    }
    fn theme() -> Self {
        Self {
            accent_color: Color32::from_rgb(191, 166, 242),
            header_bg: Color32::from_rgb(42, 40, 52),
        }
    }
    fn scripting() -> Self {
        Self {
            accent_color: Color32::from_rgb(100, 200, 140),
    ...
    ui.scope(|ui| {
        egui::Frame::new()
            ...
            .show(ui, |ui| {
                        ...
                        egui::Frame::new()
                            ...
                            .show(ui, |ui| {
                                ui.horizontal(|ui| {
                                    ...
                                    ui.label(
                                        RichText::new(icon).size(15.0).color(style.accent_color),
                                    );
                                    ui.add_space(4.0);
                                    ui.label(
                                        RichText::new(label)
                ...
                let header_response =
                    ui.interact(header_rect, id.with("header"), egui::Sense::click());
...
fn render_project_tab(
    ...
    render_category(
        ...
        |ui| {
            ...
            let thumbs_dir =
                project_path.map(|p| p.join(".cache").join("thumbnails").join("materials"));
            let cache_info = thumbs_dir.as_deref().map(cache_stats).unwrap_or_default();
            ...
            settings_row(ui, 0, "Material Thumbnails", theme, |ui| {
                let label = if cache_info.file_count > 0 {
                    format!(
                        ...
                        format_bytes(cache_info.total_bytes)
                    )
                } else {
                    "Empty".to_string()
                };
    ...
    render_category(
        ...
        |ui| {
            settings_row(ui, 0, "Parent Stacking", theme, |ui| {
                ui.checkbox(
                    &mut settings.hierarchy_parent_stacking,
                    "Pin expanded ancestor rows when scrolling",
                )
            });
        },
    );
    ...
    render_category(
        ...
        |ui| {
            settings_row(ui, 0, "Hot Reload", theme, |ui| {
                ui.checkbox(
                    &mut settings.script_rerun_on_ready_on_reload,
                    "Re-run on_ready on hot reload",
                )
            });
            settings_row(ui, 1, "Script Camera", theme, |ui| {
                ui.checkbox(
                    &mut settings.scripts_use_game_camera,
                    "Use game camera when running scripts",
                )
            });
            settings_row(ui, 2, "Cursor", theme, |ui| {
                ui.checkbox(
                    &mut settings.hide_cursor_in_play_mode,
    ...
    render_category(
        ...
        |ui| {
            settings_row(ui, 0, "Drop Import", theme, |ui| {
                ui.checkbox(
                    &mut settings.auto_import_on_drop,
                    "Auto-import on drop (skip import overlay)",
                )
            });
        },
    );
    ...
    render_category(
        ...
        |ui| {
            settings_row(ui, 0, "VSync", theme, |ui| {
                ui.checkbox(
                    &mut viewport.vsync,
                    "Cap framerate at monitor refresh rate (off = uncapped, may tear)",
                )
            });
        },
    );
    ...
    render_category(
        ...
        |ui| {
            settings_row(ui, 0, "Move Speed", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut viewport.camera.move_speed)
                        .range(1.0..=50.0)
                        .speed(0.1),
                )
            });
            settings_row(ui, 1, "Look Sensitivity", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut viewport.camera.look_sensitivity)
                        .range(0.05..=2.0)
                        .speed(0.01),
                )
            });
            settings_row(ui, 2, "Orbit Sensitivity", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut viewport.camera.orbit_sensitivity)
                        .range(0.05..=2.0)
                        .speed(0.01),
                )
            });
            settings_row(ui, 3, "Pan Sensitivity", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut viewport.camera.pan_sensitivity)
                        .range(0.1..=5.0)
                        .speed(0.01),
                )
            });
            settings_row(ui, 4, "Zoom Sensitivity", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut viewport.camera.zoom_sensitivity)
                        .range(0.1..=5.0)
                        .speed(0.01),
                )
            });
            ...
            settings_row(ui, 6, "Distance Speed", theme, |ui| {
                ui.checkbox(
                    &mut viewport.camera.distance_relative_speed,
        ...
        |ui| {
            settings_row(ui, 0, "Colliders", theme, |ui| {
                egui::ComboBox::from_id_salt("collision_gizmo_visibility")
                    ...
                    .show_ui(ui, |ui| {
                        ui.selectable_value(
                            &mut viewport.collision_gizmo_visibility,
...
fn render_shortcuts_tab(
    ...
    for (category, entries) in plugin_by_cat {
        render_category(
            ...
            category,
            CategoryStyle::shortcuts(),
            &format!("shortcuts_plugin_{}", category),
            true,
            theme,
            |ui| {
...
fn render_keybinding_row(
    ...
    egui::Frame::new()
        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ui.add(
                    egui::Label::new(RichText::new(action.display_name()).size(12.0))
...
fn render_plugin_keybinding_row(
    ...
    egui::Frame::new()
        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ui.add(
                    egui::Label::new(RichText::new(entry.display_name).size(12.0))
                        .wrap_mode(egui::TextWrapMode::Extend),
                );
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    if ui.add(button).clicked() {
                        ...
                        keybindings.plugin_rebinding =
                            if is_rebinding { None } else { Some(entry.id) };
                    }
                });
            });
        });
...
fn capture_key_for_plugin_rebind(
    ctx: &egui::Context,
    keybindings: &mut KeyBindings,
    id: &'static str,
) {
    ...
    render_category(
        ...
        |ui| {
            ...
            ui.horizontal(|ui| {
                if ui
                    .add(
                        egui::Button::new(RichText::new("Save").color(text_primary).size(11.0))
                            .fill(item_bg)
                            .stroke(Stroke::new(1.0, border_color))
                            .corner_radius(CornerRadius::same(4)),
                    )
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    if ui
                        ...
                        .changed()
                    {
                        *color = renzora_theme::ThemeColor::with_alpha(
...
fn render_input_tab(
    ...
    render_category(
        ...
        |ui| {
            ...
            ui.add_space(4.0);
            ui.label(
                RichText::new("Select an action below to edit its bindings.")
                    .size(11.5)
                    .color(muted),
            );
        },
    ...
    render_category(
        ...
        |ui| {
            ui.horizontal(|ui| {
                ...
                ui.add(
                    egui::TextEdit::singleline(&mut ui_state.new_action_name)
                        .desired_width(180.0)
                        .hint_text("Action name..."),
                );

                if ui.button(RichText::new("Button").size(11.0)).clicked()
                    && !ui_state.new_action_name.is_empty()
                {
                    let name = std::mem::take(&mut ui_state.new_action_name);
                    input_map.add(InputAction::button(name, vec![]));
                }
                if ui.button(RichText::new("Axis2D").size(11.0)).clicked()
                    && !ui_state.new_action_name.is_empty()
                {
                    let name = std::mem::take(&mut ui_state.new_action_name);
                        ...
                        .show(ui, |ui| {
                            ...
                            if kind_is_axis {
                                ui.horizontal(|ui| {
                                    ...
                                    ui.add(
                                        egui::Slider::new(&mut action.dead_zone, 0.0..=0.5)
                                            .step_by(0.01),
                                    );
                                });
                            }
                                        ...
                                        ui.with_layout(
                                            ...
                                            |ui| {
                                                if ui
                                                    .add(
                                                        egui::Button::new(
                                                            RichText::new(TRASH).size(12.0).color(
                                                                theme.semantic.error.to_color32(),
                                                            ),
                                                        )
                                                        .fill(Color32::TRANSPARENT)
                            ...
                            if ui_state.listening {
                                ui.horizontal(|ui| {
                                    ui.label(
                                        RichText::new(
                                            "Press any key, mouse button, or gamepad button...",
                                        )
                                        .size(11.5)
                                        .color(accent_color)
                                        .italics(),
                                    ...
                                    for event in &input.events {
                                        match event {
                                            ...
                                            egui::Event::PointerButton {
                                                ...
                                                return Some(InputBinding::MouseButton(
                                                    mb_str.into(),
                                                ));
                                            }
                                            _ => {}
                                ...
                                let final_binding =
                                    captured.or_else(|| ui_state.captured_gamepad.take());
                                ...
                                ui.horizontal(|ui| {
                                    if ui
                                        .button(
                                            RichText::new(format!("{} Add Binding", PLUS))
                                                .size(11.0),
                                        )
                                        .clicked()
                                    {
...
fn render_plugins_tab(ui: &mut egui::Ui, settings: &mut EditorSettings, theme: &Theme) {
    ...
    ui.add_space(8.0);
    ui.label(
        RichText::new("Plugins Directory")
            .size(13.0)
            .color(text_primary),
    );
    ui.add_space(4.0);
    ...
    ui.add_space(4.0);
    ui.label(
        RichText::new("Restart the editor to load plugins from a new directory.")
            .size(11.0)
            .color(text_muted),
    );
}
