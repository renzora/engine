...
pub fn draw_node(
    ...
    if let Some(tex_id) = node.thumbnail {
        let pad = 4.0 * zoom;
        let thumb_top = pin_start_y + (input_count as f32) * pin_spacing;
        let available_h = node_rect.max.y - thumb_top - pad;
        let half_w = scaled_w * 0.5 - pad;
        let ts = available_h.min(half_w).max(0.0);
        if ts > 4.0 {
            // Anchor to bottom so it aligns with the last output pin
            let thumb_rect = Rect::from_min_size(
                Pos2::new(screen_pos.x + pad, node_rect.max.y - ts - pad),
                Vec2::new(ts, ts),
            );
            painter.rect_filled(thumb_rect, 2.0 * zoom, Color32::from_rgb(15, 15, 18));
            painter.image(
                tex_id,
                thumb_rect,
            ...
            painter.rect_stroke(
                thumb_rect,
                2.0 * zoom,
                Stroke::new(1.0 * zoom, Color32::from_rgb(50, 50, 55)),
                StrokeKind::Inside,
            );
        }
    }
