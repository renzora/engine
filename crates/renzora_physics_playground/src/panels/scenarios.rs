...
pub fn render_scenarios_content(ui: &mut egui::Ui, state: &mut ScenariosState, theme: &Theme) {
    ...
    ui.horizontal(|ui| {
        ui.add_space(padding);
        ui.label(RichText::new("Scenarios").size(13.0).color(text_primary).strong());
        if state.alive_count > 0 {
            ui.label(RichText::new(format!("({} entities)", state.alive_count)).size(10.0).color(text_muted));
        }
    });
    ...
    ui.horizontal(|ui| {
        ...
        ui.add(egui::Slider::new(&mut state.scale, 0.5..=3.0).step_by(0.1).suffix("x"));
        if state.alive_count > 0 {
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        ui.vertical(|ui| {
            ...
            let scenarios = ScenarioType::ALL;
            let rows = (scenarios.len() + cols - 1) / cols;
            for row in 0..rows {
                ui.horizontal(|ui| {
                    ui.add_space(padding);
                    ...
                    for col in 0..cols {
                        ...
                        ui.painter().text(
                            ...
                            if selected { accent } else if hovered { accent } else { text_primary },
                        );
                        ...
                        response.clone().on_hover_text(scenario.description());

                        if response.clicked() {
                            let changed = state.selected_scenario != *scenario;
                            state.selected_scenario = *scenario;
                            if changed || state.alive_count == 0 {
                                state.commands.push(ScenarioCommand::ClearScenario);
