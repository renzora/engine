...
impl MaterialExtension for StandardVoxelMaterial {
    ...
    fn specialize(
        ...
        // Voxel meshes don't include tangent data, so strip the VERTEX_TANGENTS
        // shader def that Bevy's PBR pipeline adds. Without this, the vertex
        // shader tries to read @location(4) tangent from a buffer that doesn't
        // provide it.
        descriptor.vertex.shader_defs.retain(|d| {
            !matches!(d, ShaderDefVal::Bool(name, true) if name == "VERTEX_TANGENTS")
        });
        if let Some(ref mut frag) = descriptor.fragment {
            frag.shader_defs.retain(|d| {
                !matches!(d, ShaderDefVal::Bool(name, true) if name == "VERTEX_TANGENTS")
            });
        }
