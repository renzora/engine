...
pub fn begin_xr_session(world: &mut World) {
    ...
    let began = if let Some(session) = world.get_resource::<OxrSession>() {
        match session.begin(openxr::ViewConfigurationType::PRIMARY_STEREO) {
            Ok(_) => true,
            Err(e) => {
                error!("Failed to begin XR session: {e}");
                false
            }
        }
    } else {
...
pub fn end_xr_session(world: &mut World) {
    ...
    if let Some(session) = world.get_resource::<OxrSession>() {
        match session.end() {
            Ok(_) => {}
            Err(openxr::sys::Result::ERROR_SESSION_NOT_RUNNING) => {
                warn!("XR session end skipped — session was not running (headset removed?)");
            }
            Err(e) => {
                error!("Failed to end XR session: {e}");
            }
        }
    }
...
pub fn request_exit_xr_session(session: Res<OxrSession>) {
    if let Err(e) = session.request_exit() {
        warn!("Failed to request XR session exit: {e}");
    }
}
