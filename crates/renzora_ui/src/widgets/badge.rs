...
pub fn badge(
    ...
    let text_w = ui.fonts_mut(|f| {
        f.layout_no_wrap(text.to_string(), font.clone(), base)
            .rect
            .width()
    });
    ...
    ui.painter()
        .text(rect.center(), egui::Align2::CENTER_CENTER, text, font, base);
    resp
}
...
pub fn kbd(ui: &mut egui::Ui, key: &str, theme: &Theme) -> egui::Response {
    ...
    let text_w = ui.fonts_mut(|f| {
        f.layout_no_wrap(key.to_string(), font.clone(), text_color)
            .rect
            .width()
    });
    let (rect, resp) =
        ui.allocate_exact_size(Vec2::new(text_w.max(14.0) + 8.0, 16.0), Sense::hover());
    ui.painter()
        .rect_filled(rect, 3.0, theme.surfaces.faint.to_color32());
