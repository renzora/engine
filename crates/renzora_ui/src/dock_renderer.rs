...
pub struct DockRenderResult {
    ...
    /// User picked a panel from the empty-workspace prompt — the consumer
    /// should replace `DockingState::tree` with a single leaf containing
    /// this panel id.
    pub panel_to_seed_empty: Option<String>,
...
fn render_node(
    ...
    match node {
        DockTree::Split {
            ...
            render_split(
                ui, *direction, *ratio, first, second, rect, registry, world, base_id, path,
                result, theme, drag,
            );
        }
        ...
        DockTree::Empty => {
            ui.painter()
                .rect_filled(rect, 0.0, theme.surfaces.extreme.to_color32());

            // Empty workspace prompt — center an "Add Panel" button.
            let btn_id = base_id.with("empty_add_panel");
            let label = format!("{}  Add Panel", egui_phosphor::regular::PLUS);
            ...
            // Hint label below the button.
            ui.painter().text(
                Pos2::new(rect.center().x, btn_rect.max.y + 14.0),
                egui::Align2::CENTER_CENTER,
                "This workspace is empty. Add a panel to get started.",
                egui::FontId::proportional(11.0),
                theme.text.muted.to_color32(),
            );

            let popup_id = btn_id.with("popup");
            if resp.clicked() {
                #[allow(deprecated)]
                ui.memory_mut(|m| m.toggle_popup(popup_id));
            }
            let already_here: std::collections::HashSet<&str> = std::collections::HashSet::new();
            if let Some(picked) =
                render_panel_picker_popup(ui, popup_id, &resp, registry, theme, &already_here)
            {
                result.panel_to_seed_empty = Some(picked);
            }
        }
    }
}
...
fn calculate_split_rects(direction: SplitDirection, ratio: f32, rect: Rect) -> (Rect, Rect, Rect) {
    ...
    match direction {
        SplitDirection::Horizontal => {
            ...
            let first_rect =
                Rect::from_min_size(rect.min, Vec2::new(first_visible_w, rect.height()));
        ...
        SplitDirection::Vertical => {
            ...
            let first_rect =
                Rect::from_min_size(rect.min, Vec2::new(rect.width(), first_visible_h));
            let handle_rect = Rect::from_min_size(
...
fn render_split(
    ...
    let first_parent = ParentSplitInfo {
        direction,
        is_second: false,
        ratio,
        total,
    };
    let second_parent = ParentSplitInfo {
        direction,
        is_second: true,
        ratio,
        total,
    ...
    ui.painter()
        .rect_filled(interact_rect, 0.0, theme.surfaces.extreme.to_color32());
...
fn render_leaf(
    ...
    ui.painter()
        .rect_filled(tab_bar_rect, 0.0, theme.panels.tab_inactive.to_color32());
    ...
    for (idx, panel_id) in tabs.iter().enumerate() {
        ...
        let tab_width =
            tab_padding + icon_width + icon_gap + text_width + close_space + tab_padding;
        ...
        ui.painter()
            .rect_filled(tab_rect, egui::CornerRadius::ZERO, bg);
    ...
    if !is_dragging {
        ...
        let plus_rect = Rect::from_min_size(Pos2::new(tab_x + 2.0, rect.min.y + 2.0), plus_size);
        ...
        if ui.is_rect_visible(plus_rect) {
            ...
            ui.painter()
                .rect_filled(plus_rect, egui::CornerRadius::ZERO, bg);
            ui.painter().text(
                plus_rect.center(),
                egui::Align2::CENTER_CENTER,
                "+",
        ...
        if let Some(picked) =
            render_panel_picker_popup(ui, popup_id, &plus_resp, registry, theme, &already_here)
        {
            if let Some(sibling) = tabs.first() {
                result.panel_to_add = Some((sibling.clone(), picked));
            }
        }
    }
        ...
        if aligned && !is_dragging && !path.is_empty() {
            ...
            if empty_rect.width() > 8.0 {
                ...
                if resp.dragged() && ps.total > 0.0 {
                    ...
                    if (new_ratio - ps.ratio).abs() > 0.001 && result.ratio_update.is_none() {
    ...
    // --- Panel content area ---
    let content_rect =
        Rect::from_min_max(Pos2::new(rect.min.x, rect.min.y + TAB_BAR_HEIGHT), rect.max);

    // Background fill for content area
    ui.painter()
        .rect_filled(content_rect, 0.0, theme.surfaces.panel.to_color32());
...
// ── Panel picker popup ──────────────────────────────────────────────────────
//
// Shared between the "+" button on a leaf's tab bar and the empty-workspace
// "Add Panel" prompt. Returns the picked panel id when the user clicks one;
// callers decide what to do with it (insert as sibling vs seed an empty
// workspace).
fn render_panel_picker_popup(
    ui: &mut egui::Ui,
    popup_id: Id,
    anchor: &egui::Response,
    registry: &PanelRegistry,
    theme: &Theme,
    already_here: &std::collections::HashSet<&str>,
) -> Option<String> {
    ...
    egui::popup_below_widget(
        ...
        |ui| {
            ...
            let opened_id = popup_id.with("opened");
            let was_open = ui.memory(|m| m.data.get_temp::<bool>(opened_id).unwrap_or(false));
            if !was_open {
                        ...
                        for (col_idx, col_cats) in columns.iter().enumerate() {
                            ...
                            ui.allocate_ui_with_layout(
                                ...
                                |ui| {
                                    ...
                                    for (cat_idx, (category, panels)) in col_cats.iter().enumerate()
                                    {
                                        if cat_idx > 0 {
                                            ui.add_space(10.0);
                                        }
                                        ...
                                        for panel in sorted {
                                            ...
                                            let text_color =
                                                if is_here { disabled } else { primary };
                                            ui.painter().text(
                                                Pos2::new(
