...
pub fn derive_inspectable(input: DeriveInput) -> syn::Result<TokenStream> {
    ...
    for field in fields {
        ...
        let get_fn = match ft {
            ...
            "Vec3" => quote! {
                |world, entity| world.get::<#struct_name>(entity).map(|s| renzora::FieldValue::Vec3(s.#field_ident.into()))
            },
            "String" => quote! {
                |world, entity| world.get::<#struct_name>(entity).map(|s| renzora::FieldValue::String(s.#field_ident.clone()))
            },
            "Color" => quote! {
                |world, entity| world.get::<#struct_name>(entity).map(|s| renzora::FieldValue::Color(s.#field_ident.into()))
            },
            _ => quote! {
        ...
        let set_fn = match ft {
            ...
            "Vec3" => quote! {
                |world, entity, val| {
                    if let renzora::FieldValue::Vec3(v) = val {
            ...
            "String" => quote! {
                |world, entity, val| {
                    if let renzora::FieldValue::String(v) = val {
                        if let Some(mut s) = world.get_mut::<#struct_name>(entity) { s.#field_ident = v; }
                    }
                }
            },
            "Color" => quote! {
                |world, entity, val| {
                    if let renzora::FieldValue::Color(v) = val {
                        if let Some(mut s) = world.get_mut::<#struct_name>(entity) { s.#field_ident = v.into(); }
                    }
                }
            },
