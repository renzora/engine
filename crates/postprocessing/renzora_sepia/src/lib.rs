...
fn inspector_entry() -> InspectorEntry {
    InspectorEntry {
        ...
        has_fn: |world, entity| world.get::<SepiaSettings>(entity).is_some(),
        add_fn: Some(|world, entity| { world.entity_mut(entity).insert(SepiaSettings::default()); }),
        remove_fn: Some(|world, entity| { world.entity_mut(entity).remove::<SepiaSettings>(); }),
        is_enabled_fn: Some(|world, entity| world.get::<SepiaSettings>(entity).map(|s| s.enabled > 0.5).unwrap_or(false)),
        set_enabled_fn: Some(|world, entity, val| { if let Some(mut s) = world.get_mut::<SepiaSettings>(entity) { s.enabled = if val { 1.0 } else { 0.0 }; } }),
        fields: vec![
            FieldDef {
                name: "Intensity",
                field_type: FieldType::Float { speed: 0.01, min: 0.0, max: 1.0 },
                get_fn: |world, entity| world.get::<SepiaSettings>(entity).map(|s| FieldValue::Float(s.intensity)),
                set_fn: |world, entity, val| { if let FieldValue::Float(v) = val { if let Some(mut s) = world.get_mut::<SepiaSettings>(entity) { s.intensity = v; } } },
            },
        ],
        custom_ui_fn: None,
    }
