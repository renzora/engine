...
fn cube_rot(
    ...
    spawn_level_mesh(
        commands, meshes, materials, name, "cube", color, pos, size, rotation,
    );
}
...
fn cyl(
    ...
    spawn_level_mesh(
        ...
        color,
        pos,
        Vec3::new(diameter, height, diameter),
        Quat::IDENTITY,
    );
}
...
fn spawn_fps(
    ...
    for (pos, size) in [
        (
            Vec3::new(0.0, wall_h * 0.5, room),
            Vec3::new(room * 2.0, wall_h, wall_t),
        ),
        (
            Vec3::new(0.0, wall_h * 0.5, -room),
            Vec3::new(room * 2.0, wall_h, wall_t),
        ...
        (
            Vec3::new(room, wall_h * 0.5, 0.0),
            Vec3::new(wall_t, wall_h, room * 2.0),
        ),
        (
            Vec3::new(-room, wall_h * 0.5, 0.0),
            Vec3::new(wall_t, wall_h, room * 2.0),
    ...
    cyl(
        ...
        "Pillar",
        PILLAR_COLOR,
        Vec3::new(4.0 * s, wall_h * 0.5, -2.0 * s),
        1.2 * s,
        wall_h,
    );
...
fn spawn_third_person(
    ...
    cube(
        ...
        "Ground",
        FLOOR_COLOR,
        Vec3::new(0.0, -0.15 * s, 0.0),
        Vec3::new(size * 2.0, 0.3 * s, size * 2.0),
    );
    ...
    for (pos, sz) in [
        (
            Vec3::new(0.0, wh * 0.5, size),
            Vec3::new(size * 2.0, wh, wt),
        ),
        (
            Vec3::new(0.0, wh * 0.5, -size),
            Vec3::new(size * 2.0, wh, wt),
        ...
        (
            Vec3::new(size, wh * 0.5, 0.0),
            Vec3::new(wt, wh, size * 2.0),
        ),
        (
            Vec3::new(-size, wh * 0.5, 0.0),
            Vec3::new(wt, wh, size * 2.0),
...
fn spawn_platformer(
    ...
    for (i, (x, y, z, w, d)) in platforms.iter().enumerate() {
        let name = if i == platforms.len() - 1 {
            "End Platform"
        } else {
            "Platform"
        };
        let color = if i == platforms.len() - 1 {
            ACCENT_COLOR
        } else {
            PLATFORM_COLOR
        };
...
fn spawn_top_down(
    ...
    cube(
        ...
        FLOOR_COLOR,
        Vec3::new(0.0, -0.15 * s, 0.0),
        Vec3::new(grid * 2.0, 0.3 * s, grid * 2.0),
    );
    ...
    for (pos, sz) in [
        (
            Vec3::new(0.0, wall_h * 0.5, grid),
            Vec3::new(grid * 2.0, wall_h, wall_t),
        ),
        (
            Vec3::new(0.0, wall_h * 0.5, -grid),
            Vec3::new(grid * 2.0, wall_h, wall_t),
        ...
        (
            Vec3::new(grid, wall_h * 0.5, 0.0),
            Vec3::new(wall_t, wall_h, grid * 2.0),
        ),
        (
            Vec3::new(-grid, wall_h * 0.5, 0.0),
            Vec3::new(wall_t, wall_h, grid * 2.0),
    ...
    for row in [-1, 0, 1] {
        ...
        for col in [-1, 1] {
            ...
            cube(
                ...
                Vec3::new(segment, wall_h, wall_t),
            );
        }
    }
    // Vertical walls
    for col in [-1, 0, 1] {
        ...
        for row in [-1, 1] {
            ...
            cube(
                ...
                WALL_COLOR,
                Vec3::new(x, wall_h * 0.5, z),
                Vec3::new(wall_t, wall_h, segment),
            );
        }
    }
...
fn spawn_racing(
    ...
    cube(
        ...
        materials,
        "Ground",
        Color::srgb(0.28, 0.35, 0.25),
        Vec3::new(0.0, -0.1 * s, 0.0),
        Vec3::new(100.0 * s, 0.2 * s, 80.0 * s),
    );
    ...
    cube(
        ...
        Vec3::new(straight, 0.15 * s, track_w),
    );
    cube(
        commands,
        meshes,
...
fn spawn_sandbox(
    ...
    cube(
        ...
        FLOOR_COLOR,
        Vec3::new(0.0, -0.15 * s, 0.0),
        Vec3::new(100.0 * s, 0.3 * s, 100.0 * s),
    );
...
fn spawn_corridor(
    ...
    cube(
        ...
        "Floor",
        FLOOR_COLOR,
        Vec3::new(-leg1 * 0.5, -wall_t * 0.5, 0.0),
        Vec3::new(leg1, wall_t, hall_w),
    );
    cube(
        commands,
        meshes,
    ...
    cube(
        ...
        "Floor",
        FLOOR_COLOR,
        Vec3::new(0.0, -wall_t * 0.5, leg2 * 0.5),
        Vec3::new(hall_w, wall_t, leg2),
    );
    ...
    cube(
        ...
        "End Wall",
        WALL_COLOR,
        Vec3::new(-leg1, wall_h * 0.5, 0.0),
        Vec3::new(hall_w + wall_t, wall_h, wall_t),
    );
    cube(
        commands,
        meshes,
    ...
    for x in [-leg1 * 0.7, -leg1 * 0.3] {
        spawn_level_point_light(
            commands,
            Vec3::new(x, wall_h * 0.8, 0.0),
...
fn spawn_arena(
    ...
    cube(
        ...
        FLOOR_COLOR,
        Vec3::new(0.0, -0.15 * s, 0.0),
        Vec3::new(radius * 2.2, 0.3 * s, radius * 2.2),
    );
...
fn spawn_showcase(
    ...
    spawn_level_point_light(
        commands,
        Vec3::new(0.0, 6.0 * s, -4.0 * s),
        Color::srgb(0.8, 0.85, 1.0),
        8000.0,
    );
...
fn spawn_terrain(
    ...
    cube(
        ...
        materials,
        "Ground",
        Color::srgb(0.30, 0.38, 0.25),
        Vec3::new(0.0, -0.15 * s, 0.0),
        Vec3::new(60.0 * s, 0.3 * s, 60.0 * s),
    );
        ...
        if i % 2 == 0 {
            for t in 0..3u32 {
                ...
                cyl(
                    ...
                    materials,
                    "Trunk",
                    Color::srgb(0.45, 0.30, 0.20),
                    Vec3::new(tx * s, *h * s + 1.0 * s, tz * s),
                    0.3 * s,
                    2.0 * s,
                );
                ...
                sphere(
                    ...
                    materials,
                    "Canopy",
                    Color::srgb(0.20, 0.45, 0.20),
                    Vec3::new(tx * s, *h * s + 2.5 * s, tz * s),
                    2.0 * s,
                );
            }
        }
    ...
    cube(
        ...
        materials,
        "Valley Floor",
        Color::srgb(0.25, 0.32, 0.22),
        Vec3::new(2.0 * s, -0.2 * s, 0.0),
        Vec3::new(8.0 * s, 0.1 * s, 12.0 * s),
    );
