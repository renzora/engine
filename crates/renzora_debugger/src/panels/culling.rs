...
fn render_culling_breakdown(ui: &mut egui::Ui, state: &CullingDebugState, theme: &Theme) {
    ui.label(
        RichText::new("Culling Breakdown")
            .size(12.0)
            .color(theme.text.muted.to_color32()),
    );
    ui.add_space(4.0);
    ...
    egui::Frame::NONE
        ...
        .show(ui, |ui| {
            ...
            let visible_after_distance =
                state.frustum_visible.saturating_sub(state.distance_culled);

            render_bar_row(
                ui,
                "Visible",
                visible_after_distance,
                total,
...
fn render_bar_row(
    ...
    let (rect, _) =
        ui.allocate_exact_size(egui::Vec2::new(available_width, 6.0), egui::Sense::hover());
    ui.painter()
        .rect_filled(rect, 2.0, theme.surfaces.extreme.to_color32());
    let filled =
        egui::Rect::from_min_size(rect.min, egui::Vec2::new(available_width * fraction, 6.0));
...
fn render_distance_distribution(ui: &mut egui::Ui, state: &CullingDebugState, theme: &Theme) {
    ui.label(
        RichText::new("Distance Distribution")
            .size(12.0)
            .color(theme.text.muted.to_color32()),
    );
    ui.add_space(4.0);
        ...
        .show(ui, |ui| {
                ...
                .enumerate()
            {
                ...
                let (rect, _) = ui.allocate_exact_size(
                    egui::Vec2::new(available_width, 4.0),
                    egui::Sense::hover(),
                );
                ui.painter()
                    .rect_filled(rect, 2.0, theme.surfaces.extreme.to_color32());
                ...
                let bar_color =
                    Color32::from_rgb((80.0 + 120.0 * ci) as u8, (160.0 - 60.0 * ci) as u8, 200);
                let filled = egui::Rect::from_min_size(
                    rect.min,
                    egui::Vec2::new(available_width * fraction, 4.0),
                );
    ...
    egui::Frame::NONE
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.add(
                    egui::Slider::new(&mut state.fade_start_fraction, 0.5..=1.0).fixed_decimals(2),
                );
            });
        });
}
