...
fn render_asset_memory_section(ui: &mut egui::Ui, state: &MemoryProfilerState, theme: &Theme) {
    ui.label(
        RichText::new("Asset Memory (Estimated)")
            .size(12.0)
            .color(theme.text.muted.to_color32()),
    );
    ui.add_space(4.0);
    ...
    let max_bytes = assets
        .meshes_bytes
        .max(assets.textures_bytes)
        .max(assets.materials_bytes)
        .max(1);
...
fn render_memory_graph(ui: &mut egui::Ui, state: &MemoryProfilerState, theme: &Theme) {
    ...
    if points.len() >= 2 {
        ...
        painter.add(egui::Shape::convex_polygon(
            fill_points,
            Color32::from_rgba_unmultiplied(150, 100, 200, 30),
            Stroke::NONE,
        ));
