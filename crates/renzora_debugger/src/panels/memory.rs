            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                if !state.available {
                    ui.vertical_centered(|ui| {
                        ui.add_space(40.0);
                        ui.label(RichText::new("Memory profiling unavailable").size(14.0).color(theme.text.muted.to_color32()));
                    });
                    return;
                }
...
fn render_process_memory_section(ui: &mut egui::Ui, state: &MemoryProfilerState, theme: &Theme) {
    ...
    ui.horizontal(|ui| {
        ui.label(RichText::new(format_memory(state.process_memory)).size(28.0).color(memory_color).strong());
    });

    ui.add_space(4.0);
    ui.horizontal(|ui| {
        ui.label(RichText::new("Peak:").size(11.0).color(theme.text.secondary.to_color32()));
        ui.label(RichText::new(format_memory(state.peak_memory)).size(11.0).color(theme.text.primary.to_color32()));
    });
...
fn render_memory_trend_section(ui: &mut egui::Ui, state: &MemoryProfilerState, theme: &Theme) {
    ui.horizontal(|ui| {
        ...
        let (icon, color, text) = match state.memory_trend {
            MemoryTrend::Increasing => ("\u{2191}", Color32::from_rgb(220, 100, 100), "Increasing"),
            MemoryTrend::Decreasing => ("\u{2193}", Color32::from_rgb(100, 200, 100), "Decreasing"),
            MemoryTrend::Stable => ("\u{2194}", Color32::from_gray(150), "Stable"),
        };
...
fn render_asset_memory_section(ui: &mut egui::Ui, state: &MemoryProfilerState, theme: &Theme) {
    ...
    ui.horizontal(|ui| {
        ui.label(RichText::new(format_memory(total)).size(20.0).color(theme.text.primary.to_color32()).strong());
...
fn render_asset_bar(ui: &mut egui::Ui, name: &str, count: usize, bytes: u64, max_bytes: u64, color: Color32, theme: &Theme) {
    ui.label(RichText::new(format!("{} ({})", name, count)).size(10.0).color(theme.text.secondary.to_color32()));
    ...
    ui.horizontal(|ui| {
        ...
        let fill_rect = egui::Rect::from_min_size(rect.min, Vec2::new(rect.width() * fill_ratio, rect.height()));
        ui.painter().rect_filled(fill_rect, 2.0, color);
        ui.add_space(8.0);
        ui.label(RichText::new(format_memory(bytes)).size(10.0).color(theme.text.primary.to_color32()));
    });
}
...
fn render_allocation_rate_section(ui: &mut egui::Ui, state: &MemoryProfilerState, theme: &Theme) {
    ...
    ui.horizontal(|ui| {
        if rate > 0.0 { ui.label(RichText::new("+").size(14.0).color(color)); }
        else if rate < 0.0 { ui.label(RichText::new("-").size(14.0).color(color)); }
    ...
    if rate > 10.0 * 1024.0 * 1024.0 {
        ...
        ui.horizontal(|ui| {
            ...
            ui.label(RichText::new("High allocation rate detected").size(10.0).color(Color32::from_rgb(220, 180, 80)));
        });
    }
}
...
fn render_memory_graph(ui: &mut egui::Ui, state: &MemoryProfilerState, theme: &Theme) {
    ...
    if points.len() >= 2 {
        ...
        painter.add(egui::Shape::convex_polygon(fill_points, Color32::from_rgba_unmultiplied(150, 100, 200, 30), Stroke::NONE));
...
fn memory_to_color(mb: f64) -> Color32 {
    if mb < 512.0 { Color32::from_rgb(100, 200, 100) }
    else if mb < 1024.0 { Color32::from_rgb(200, 200, 100) }
    else if mb < 2048.0 { Color32::from_rgb(200, 150, 80) }
