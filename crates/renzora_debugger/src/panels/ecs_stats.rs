...
pub fn render_ecs_stats_content(ui: &mut egui::Ui, state: &mut EcsStatsState, theme: &Theme) {
    egui::Frame::NONE
...
fn render_entity_graph(ui: &mut egui::Ui, state: &EcsStatsState, theme: &Theme) {
    ...
    if points.len() >= 2 {
        ...
        painter.add(egui::Shape::convex_polygon(
            fill_points,
            Color32::from_rgba_unmultiplied(100, 180, 220, 30),
            Stroke::NONE,
        ));
...
fn render_archetype_section(ui: &mut egui::Ui, state: &mut EcsStatsState, theme: &Theme) {
    let header_response = ui.horizontal(|ui| {
        let icon = if state.show_archetypes {
...
fn render_component_section(ui: &mut egui::Ui, state: &mut EcsStatsState, theme: &Theme) {
    let header_response = ui.horizontal(|ui| {
        let icon = if state.show_components {
            egui_phosphor::regular::CARET_DOWN
        } else {
            egui_phosphor::regular::CARET_RIGHT
        };
                ...
                if state.component_stats.is_empty() {
                    ...
                    for comp in state.component_stats.iter().take(30) {
                        ui.horizontal(|ui| {
                            ...
                            ui.add_space(8.0);
                            ui.label(
                                RichText::new(short_name(&comp.name))
                                    .size(10.0)
                                    .color(theme.text.secondary.to_color32()),
                            );
                        });
...
fn render_resources_section(ui: &mut egui::Ui, state: &mut EcsStatsState, theme: &Theme) {
    let header_response = ui.horizontal(|ui| {
        let icon = if state.show_resources {
            egui_phosphor::regular::CARET_DOWN
        } else {
            egui_phosphor::regular::CARET_RIGHT
        };
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                if state.resources.is_empty() {
                    ui.label(
                        RichText::new("Resource tracking not enabled")
                            .size(11.0)
                            .color(theme.text.muted.to_color32()),
                    );
                } else {
                    for resource in state.resources.iter().take(50) {
                        ui.label(
                            RichText::new(short_name(resource))
                                .size(10.0)
                                .color(theme.text.secondary.to_color32()),
                        );
                    }
