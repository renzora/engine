...
pub fn render_render_stats_content(ui: &mut egui::Ui, render_stats: &RenderStats, theme: &Theme) {
    egui::Frame::NONE
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                if !render_stats.enabled {
                    ui.vertical_centered(|ui| {
                        ui.add_space(40.0);
                        ui.label(
                            RichText::new("Collecting render data...")
                                .size(14.0)
...
fn render_gpu_section(ui: &mut egui::Ui, stats: &RenderStats, theme: &Theme) {
    ...
    ui.horizontal(|ui| {
        ui.label(
            RichText::new(format!("{:.2}", stats.gpu_time_ms))
                .size(28.0)
                .color(gpu_time_color)
                .strong(),
        );
        ui.label(
            RichText::new("ms GPU")
...
fn render_pipeline_stats_section(ui: &mut egui::Ui, stats: &RenderStats, theme: &Theme) {
    ui.label(
        RichText::new("Pipeline Statistics")
            .size(12.0)
            .color(theme.text.muted.to_color32()),
    );
    ui.add_space(4.0);
