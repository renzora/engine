...
fn render_frame_time_section(ui: &mut egui::Ui, diagnostics: &DiagnosticsState, theme: &Theme) {
    ...
    ui.horizontal(|ui| {
        ...
        if frame_time <= target_60fps {
            ui.label(
                RichText::new(format!(
                    "\u{2713} Under 60fps target ({:.1}ms budget)",
                    target_60fps - frame_time
                ))
                .size(10.0)
            ...
            ui.label(
                RichText::new(format!(
                    "\u{26a0} Between 30-60fps ({:.1}ms over 60fps target)",
                    frame_time - target_60fps
                ))
                .size(10.0)
            ...
            ui.label(
                RichText::new(format!(
                    "\u{2717} Below 30fps ({:.1}ms over target)",
                    frame_time - target_30fps
                ))
                .size(10.0)
    ...
    render_graph(
        ui,
        &diagnostics.frame_time_history,
        theme,
            ...
            egui::Grid::new("fps_stats_grid")
                ...
                .show(ui, |ui| {
                    ...
                    grid_row(
                        ui,
                        "Entities",
                        &format!("{}", diagnostics.entity_count),
                        theme,
                    );
                });
        });
...
fn render_schedule_breakdown(ui: &mut egui::Ui, timing_state: &SystemTimingState, theme: &Theme) {
    ui.label(
        RichText::new("Schedule Overview (Estimated)")
            .size(12.0)
            .color(theme.text.muted.to_color32()),
    );
    ui.add_space(4.0);
    ...
    let total_time: f32 = timing_state
        .schedule_timings
        .iter()
        .map(|s| s.time_ms)
        .sum();
        ...
        .show(ui, |ui| {
            for schedule in &timing_state.schedule_timings {
                ...
                let ratio = if total_time > 0.0 {
                    schedule.time_ms / total_time
                } else {
                    0.0
                };
                let (rect, _) =
                    ui.allocate_exact_size(Vec2::new(bar_width, 8.0), egui::Sense::hover());
...
fn render_profiler_links(ui: &mut egui::Ui, theme: &Theme) {
    ui.label(
        RichText::new("External Profilers")
            .size(12.0)
            .color(theme.text.muted.to_color32()),
    );
    ui.add_space(4.0);
...
fn render_graph(
    ...
    if points.len() >= 2 {
        let fill_color =
            Color32::from_rgba_unmultiplied(line_color.r(), line_color.g(), line_color.b(), 30);
