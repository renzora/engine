...
use super::graph::{MaterialGraph, MaterialNode, MaterialDomain, NodeId, PinValue};
use super::nodes;
...
impl<'a> Ctx<'a> {
    ...
    fn pin_type_for(node_type: &str, pin_name: &str, direction: super::graph::PinDir) -> Option<super::graph::PinType> {
    ...
    fn input(&mut self, node: &MaterialNode, pin_name: &str) -> String {
        ...
        let dest_type = Self::pin_type_for(&node.node_type, pin_name, super::graph::PinDir::Input);
        ...
        if let Some(conn) = self.graph.connection_to(node.id, pin_name) {
            ...
            if let Some(expr) = self.output_vars.get(&(from_node, from_pin.clone())).cloned() {
                ...
                if let (Some(dt), Some(src_node)) = (dest_type, self.graph.get_node(from_node)) {
                    if let Some(st) = Self::pin_type_for(&src_node.node_type, &from_pin, super::graph::PinDir::Output) {
                        return super::graph::PinType::cast_expr(st, dt, &expr);
                    }
                }
                return expr;
            }
...
pub fn compile(graph: &MaterialGraph) -> CompileResult {
    ...
    let output_pins: Vec<String> = if let Some(def) = nodes::node_def(&output_node.node_type) {
        (def.pins)()
            .iter()
            .filter(|p| p.direction == super::graph::PinDir::Input)
            .map(|p| p.name.clone())
            .collect()
    } else {
