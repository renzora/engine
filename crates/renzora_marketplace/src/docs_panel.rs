...
//! it works signed out.
//!
//! Restored from `renzora_social::panels::learn` (deleted in 2ce2d58d, which
//! removed the social crates wholesale — this panel went with them despite
//! having nothing social about it). It lives in the marketplace crate now
//! because that is where the API client it needs ended up. The routing hook it
//! used, `SocialPanelRequest::Learn`, no longer exists; the panel is opened
//! from the panel menu like any other.
...
use crate::auth::docs::{DocPage, DocSearchResult, DocVersions, Sidebar};
...
use crate::util::{self, hash64};

/// Cyan, as it was in `renzora_social::util` — the accent for the doc tree's
/// active row and group chips.
const HUE_LEARN: (u8, u8, u8) = (86, 182, 222);

pub(crate) const PANEL_ID: &str = "docs";
...
impl LearnPanel {
    ...
    fn open_page(&mut self, slug: String) {
        ...
        spawn_thread(move || {
            let _ = tx.send(LearnResult::Page(crate::auth::docs::get_page(&version, &slug)));
        });
    }
}
...
pub(crate) fn register(app: &mut App) {
    ...
    app.register_shell_panel(PANEL_ID, "Docs", "book-open", "Help");
        ...
        match r {
            LearnResult::Versions(Ok(v)) => {
                ...
                if panel.doc_version.is_none() && !default.is_empty() {
                    ...
                    spawn_thread(move || {
                        let _ = tx.send(LearnResult::Sidebar(crate::auth::docs::get_sidebar(&default)));
                    });
                }
                panel.bump();
            }
...
fn auto_load(mut panel: ResMut<LearnPanel>) {
    if !panel.loaded_once {
        ...
        let tx = panel.tx.clone();
        spawn_thread(move || {
            let _ = tx.send(LearnResult::Versions(crate::auth::docs::get_versions()));
        });
    }
}
...
fn search_debounce(
    ...
    if let Some((q, deadline)) = panel.pending_query.clone() {
        if now >= deadline {
            ...
            let tx = panel.tx.clone();
            spawn_thread(move || {
                let _ = tx.send(LearnResult::Search(crate::auth::docs::search(&version, &q)));
            });
        }
    }
}
...
fn clicks(
    ...
    for (b, dd) in &version_dropdowns {
        ...
        if panel.doc_version.as_deref() != Some(id.as_str()) {
            ...
            let tx = panel.tx.clone();
            spawn_thread(move || {
                let _ = tx.send(LearnResult::Sidebar(crate::auth::docs::get_sidebar(&version)));
            });
        }
    }
}
