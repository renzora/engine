...
pub fn register_modifiers(type_registry: &AppTypeRegistry) {
    {
        ...
        // force.rs (renzora fork additions)
        type_registry.register::<NoiseTurbulenceModifier>();
        type_registry.register::<OrbitModifier>();
        type_registry.register::<VelocityLimitModifier>();
        // kill.rs
        ...
        // output.rs (renzora fork additions)
        type_registry.register::<ErosionModifier>();
    ...
    // force.rs (renzora fork additions)
    register_reflect_modifier::<NoiseTurbulenceModifier>(type_registry, |module| {
        let frequency = module.lit(1.0);
        let amplitude = module.lit(1.0);
        Box::new(NoiseTurbulenceModifier::new(frequency, amplitude, 1, 2.0))
    });
    register_reflect_modifier::<OrbitModifier>(type_registry, |module| {
        let center = module.lit(Vec3::ZERO);
        let axis = module.lit(Vec3::Y);
        let speed = module.lit(1.0);
        let radial_pull = module.lit(0.0);
        let orbit_radius = module.lit(1.0);
        Box::new(OrbitModifier::new(
            center,
            axis,
            speed,
            radial_pull,
    ...
    register_reflect_modifier::<VelocityLimitModifier>(type_registry, |module| {
        let max_speed = module.lit(1.0);
        Box::new(VelocityLimitModifier::new(max_speed))
    });
    ...
    // output.rs (renzora fork additions)
    register_reflect_modifier::<ErosionModifier>(type_registry, |module| {
        let noise_slot = module.lit(0u32);
        Box::new(ErosionModifier::new(noise_slot))
    });
