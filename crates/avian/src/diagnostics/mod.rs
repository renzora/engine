...
// Bevy 0.19: `Resource: Component`, and `ResMut<T>` now requires
// `T: Resource<Mutability = Mutable>`. The `reset` default method takes
// `ResMut<Self>`, so the trait must guarantee its impls are mutable resources.
pub trait PhysicsDiagnostics:
    Default + Resource<Mutability = bevy::ecs::component::Mutable>
{
