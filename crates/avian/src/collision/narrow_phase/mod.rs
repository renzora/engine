...
fn trigger_collision_events(
    ...
    // Bevy 0.19: `SystemState::get_mut` is now fallible (returns a
    // `Result<_, SystemParamValidationError>`). These params (a query + two
    // message readers) always validate, so an error here is unreachable.
