//! Plugin registry for tracking registered plugin UI elements.
...
use editor_plugin_api::api::{
    ContextMenuLocation, InspectorDefinition, MenuItem, MenuLocation, PanelDefinition, ToolbarItem,
};
...
pub struct RegisteredPanel {
    ...
    /// JSON-serialized widget content.
...
impl PluginRegistry {
    ...
    /// Set panel content (JSON string).
    pub fn set_panel_content(&mut self, panel_id: &str, content_json: String) {
        if let Some(panel) = self.panels.get_mut(panel_id) {
            panel.content_json = Some(content_json);
        }
    }
