//! Mirror of the root `build.rs` — build scripts are per-package, so the editor
//! binary needs its own for the Windows icon/version resource and the `zstd`
//! link directive. Only the icon path (one level up) and the file description
//! differ.
