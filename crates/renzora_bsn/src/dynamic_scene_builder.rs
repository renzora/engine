...
impl<'w> DynamicSceneBuilder<'w> {
    ...
    ///
    /// **Interim BSN note:** resource extraction is a no-op. Renzora's scene
    /// save always calls [`deny_all_resources`](Self::deny_all_resources), and
    /// Bevy 0.19 moved resource storage (`Resource: Component`), removing the
    /// `Storages::resources` iterator the upstream port relied on. Kept as a
    /// no-op so the builder chain (and [`DynamicScene::from_world`]) is unchanged.
    #[must_use]
...
mod tests {
    ...
    // NOTE: resource-extraction tests were removed — the interim BSN format
    // does not serialize resources (see `extract_resources`, now a no-op).
