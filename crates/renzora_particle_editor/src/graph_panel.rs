...
use egui_phosphor::regular::{
    CROSSHAIR, FLOW_ARROW, MAGNIFYING_GLASS_MINUS, MAGNIFYING_GLASS_PLUS, PLUS, SPARKLE,
};
    ...
    fn ui(&self, ui: &mut egui::Ui, world: &World) {
        ...
        if editor_state.current_effect.is_none() {
            ...
            ui.vertical_centered(|ui| {
                ...
                ui.label(
                    RichText::new("Particle Graph Editor")
                        .size(20.0)
                        .color(text_muted),
                );
                ui.add_space(16.0);
                ui.label(
                    RichText::new("Load an effect in the Particle Editor panel first")
                        .size(12.0)
                        .color(text_muted),
                );
            });
        ...
        let mut node_graph = editor_state.node_graph.clone().unwrap_or_else(|| {
        ...
        let (graph_modified, selected) = GRAPH_EDITOR_STATE.with(|cell| {
            let mut graph_state = cell.borrow_mut();
            let m =
                graph_editor::render_graph_editor(ui, &mut node_graph, &mut graph_state, &theme);
...
fn render_toolbar(
    ...
    ui.horizontal(|ui| {
        ...
        let presets_btn = ui.add(egui::Button::new(
            RichText::new(format!("{} Presets", SPARKLE))
                .size(12.0)
                .color(text_color),
        ));
                        ...
                        if ui.button(format!("{} {}", icon, display)).clicked() {
                            ...
                            cmds.push(move |world: &mut World| {
                                if let Some(effect) = load_effect_from_file(&path) {
                                    ...
                                    state.node_graph =
                                        Some(ParticleNodeGraph::from_effect(&effect));
                                }
                            });
                            // Force resync
        ...
        let add_btn = ui.add(egui::Button::new(
            RichText::new(format!("{} Add Node", PLUS))
                .size(12.0)
                .color(accent),
        ));
                ...
                for &category in ParticleNodeType::categories() {
                    ...
                    ui.menu_button(format!("{} {}", icon, category), |ui| {
                        ...
                        for node_type in &nodes {
                            if ui
                                .button(format!("{} {}", icon, node_type.display_name()))
                                .clicked()
                            {
