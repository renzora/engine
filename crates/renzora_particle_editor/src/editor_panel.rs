...
use egui_phosphor::regular::{
    ARROWS_OUT, ATOM, CODE, CUBE, FLOPPY_DISK, FOLDER_OPEN, GAUGE, GEAR, MINUS, PALETTE, PLANET,
    PLUS, PROHIBIT, RESIZE, SHAPES, SPARKLE, SPIRAL, TIMER, WIND,
};
...
use renzora_hanabi::{
    load_effect_from_file, ConformToSphere, EditorMode, EffectVariable, FlipbookSettings,
    GradientStop, HanabiEffectDefinition, HanabiEmitShape, KillZone, MotionIntegrationMode,
    OrbitSettings, ParticleAlphaMode, ParticleColorBlendMode, ParticleEditorState,
    ParticleOrientMode, ShapeDimension, SimulationCondition, SimulationSpace, SpawnMode,
    VelocityMode,
};
    ...
    fn ui(&self, ui: &mut egui::Ui, world: &World) {
                ...
                cmds.push(move |world: &mut World| {
                    ...
                    if is_advanced {
                        ...
                        if let Some(ref effect) = state.current_effect {
                            state.node_graph = Some(
                                renzora_hanabi::node_graph::ParticleNodeGraph::from_effect(effect),
                            );
                        } else {
                            let name = "New Effect".to_string();
                            state.node_graph =
                                Some(renzora_hanabi::node_graph::ParticleNodeGraph::new(&name));
                        }
                    }
                    // Swap layout
        ...
        if save_as_requested {
            ...
            cmds.push(move |world: &mut World| {
                ...
                let sanitized = effect_to_save.name.replace(
...
impl ParticleEditorPanel {
    fn render_graph_inspector(
        ...
        use renzora_hanabi::node_graph::{ParticleNodeType, PinDir, PinValue};

        let text_muted = theme.text.muted.to_color32();

        let Some(ref graph) = editor_state.node_graph else {
            ui.label(
                RichText::new("No graph initialized")
                    .size(11.0)
                    .color(text_muted),
            );
            return;
        ...
        let mut all_modified_values: Vec<(u64, std::collections::HashMap<String, PinValue>)> =
            Vec::new();
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                for &(category, icon, theme_cat) in &categories {
                    let nodes_in_cat: Vec<_> = graph
                        .nodes
                        .iter()
                        .filter(|n| n.node_type.category() == category)
                        .collect();
                    if nodes_in_cat.is_empty() {
                        continue;
                    }
                    ...
                    for node in &nodes_in_cat {
                        ...
                        let node_icon = match node.node_type {
                            ...
                            | ParticleNodeType::TangentAccel => WIND,
                        ...
                        collapsible_section(
                            ...
                            |ui| {
                                ...
                                let connected_pins: Vec<String> = graph
                                    .connections
                                    .iter()
                                    ...
                                    if is_pin_connected {
                                        ...
                                        match val {
                                            PinValue::Float(v) => {
                                                inline_property(ui, row, &pin.label, theme, |ui| {
                                                    if ui
                                                        .add(egui::DragValue::new(v).speed(0.01))
                                                        .changed()
                                                    {
                                                        node_modified = true;
                                                    }
    ...
    fn render_welcome_screen(&self, ui: &mut egui::Ui, cmds: &EditorCommands, theme: &Theme) {
        ...
        ui.scope_builder(egui::UiBuilder::new().max_rect(available), |ui| {
            ui.vertical_centered(|ui| {
                ...
                ui.label(
                    RichText::new("Particle Editor")
                        .size(20.0)
                        .color(text_muted),
                );
                ui.add_space(16.0);
                ui.label(
                    RichText::new("Create or load a particle effect to begin")
                        .size(12.0)
                        .color(text_muted),
                );
                ui.add_space(24.0);
    ...
    ui.horizontal(|ui| {
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            if ui
                .add_enabled(
                    save_enabled,
                    egui::Button::new(format!("{} Save", FLOPPY_DISK)).small(),
                )
                .clicked()
            {
                save_requested = true;
    ...
    collapsible_section(
        ...
        |ui| {
            ...
            modified |= inline_property(ui, row, "Capacity", theme, |ui| {
                ui.add(egui::DragValue::new(&mut effect.capacity).range(10..=100000))
                    .changed()
            });
        },
    );
        ...
        |ui| {
            ...
            modified |= inline_property(ui, row, "Mode", theme, |ui| {
                ...
                egui::ComboBox::from_id_salt("spawn_mode_editor")
                    ...
                    .show_ui(ui, |ui| {
                        if ui
                            .selectable_label(effect.spawn_mode == SpawnMode::Rate, "Continuous")
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(effect.spawn_mode == SpawnMode::Burst, "Single Burst")
                            .clicked()
                        {
            ...
            match effect.spawn_mode {
                SpawnMode::Rate => {
                    modified |= inline_property(ui, row, "Rate/sec", theme, |ui| {
                        ui.add(
                            egui::DragValue::new(&mut effect.spawn_rate)
                                .speed(1.0)
                                .range(0.1..=10000.0),
                        )
                        .changed()
                ...
                SpawnMode::Burst => {
                    modified |= inline_property(ui, row, "Count", theme, |ui| {
                        ui.add(egui::DragValue::new(&mut effect.spawn_count).range(1..=10000))
                            .changed()
                    });
                }
                SpawnMode::BurstRate => {
            ...
            modified |= inline_property(ui, row, "Duration", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.spawn_duration)
                        .speed(0.1)
                        .range(0.0..=600.0)
                        .suffix("s"),
                )
            ...
            modified |= inline_property(ui, row, "Cycles", theme, |ui| {
                ui.add(egui::DragValue::new(&mut effect.spawn_cycle_count).range(0..=1000))
                    .changed()
            });
            row += 1;
    ...
    collapsible_section(
        ...
        |ui| {
            modified |= inline_property(ui, 0, "Min", theme, |ui| {
                let changed = ui
                    .add(
                        egui::DragValue::new(&mut effect.lifetime_min)
                            .speed(0.1)
                            .range(0.01..=60.0)
                            .suffix("s"),
                    )
            ...
            modified |= inline_property(ui, 1, "Max", theme, |ui| {
                let changed = ui
                    .add(
                        egui::DragValue::new(&mut effect.lifetime_max)
                            .speed(0.1)
                            .range(0.01..=60.0)
                            .suffix("s"),
                    )
            ...
            modified |= inline_property(ui, row, "Shape", theme, |ui| {
                ...
                egui::ComboBox::from_id_salt("emit_shape_editor")
                    ...
                    .show_ui(ui, |ui| {
                        if ui
                            .selectable_label(
                                matches!(&effect.emit_shape, HanabiEmitShape::Point),
                                "Point",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                matches!(&effect.emit_shape, HanabiEmitShape::Circle { .. }),
                                "Circle",
                            )
                            .clicked()
                        {
                            effect.emit_shape = HanabiEmitShape::Circle {
                                radius: 1.0,
                        ...
                        if ui
                            .selectable_label(
                                matches!(&effect.emit_shape, HanabiEmitShape::Sphere { .. }),
                                "Sphere",
                            )
                            .clicked()
                        {
                            effect.emit_shape = HanabiEmitShape::Sphere {
                                radius: 1.0,
                                dimension: ShapeDimension::Volume,
                            };
                            changed = true;
                        ...
                        if ui
                            .selectable_label(
                                matches!(&effect.emit_shape, HanabiEmitShape::Cone { .. }),
                                "Cone",
                            )
                            .clicked()
                        {
                            effect.emit_shape = HanabiEmitShape::Cone {
                                base_radius: 0.5,
                                top_radius: 0.0,
                                height: 1.0,
                                dimension: ShapeDimension::Volume,
                        ...
                        if ui
                            .selectable_label(
                                matches!(&effect.emit_shape, HanabiEmitShape::Rect { .. }),
                                "Rectangle",
                            )
                            .clicked()
                        {
                            effect.emit_shape = HanabiEmitShape::Rect {
                        ...
                        if ui
                            .selectable_label(
                                matches!(&effect.emit_shape, HanabiEmitShape::Box { .. }),
                                "Box",
                            )
                            .clicked()
                        {
                            effect.emit_shape = HanabiEmitShape::Box {
            ...
            match &mut effect.emit_shape {
                ...
                | HanabiEmitShape::Sphere { radius, dimension } => {
                    modified |= inline_property(ui, row, "Radius", theme, |ui| {
                        ui.add(egui::DragValue::new(radius).speed(0.1).range(0.001..=100.0))
                            .changed()
                    });
                    row += 1;
                    modified |= inline_property(ui, row, "Emit from", theme, |ui| {
                        let mut changed = false;
                        if ui
                            .radio(*dimension == ShapeDimension::Volume, "Volume")
                            .clicked()
                        {
                            *dimension = ShapeDimension::Volume;
                            changed = true;
                        ...
                        if ui
                            .radio(*dimension == ShapeDimension::Surface, "Surface")
                            .clicked()
                        {
                ...
                HanabiEmitShape::Cone {
                    ...
                    modified |= inline_property(ui, row, "Base Radius", theme, |ui| {
                        ui.add(
                            egui::DragValue::new(base_radius)
                                .speed(0.1)
                                .range(0.0..=100.0),
                        )
                        .changed()
                    ...
                    row += 1;
                    modified |= inline_property(ui, row, "Top Radius", theme, |ui| {
                        ui.add(
                            egui::DragValue::new(top_radius)
                                .speed(0.1)
                                .range(0.0..=100.0),
                        )
                        .changed()
                    ...
                    row += 1;
                    modified |= inline_property(ui, row, "Height", theme, |ui| {
                        ui.add(egui::DragValue::new(height).speed(0.1).range(0.001..=100.0))
                            .changed()
                    });
                    row += 1;
                    modified |= inline_property(ui, row, "Emit from", theme, |ui| {
                ...
                HanabiEmitShape::Rect {
                    half_extents,
                    dimension,
                } => {
                    modified |= inline_property(ui, row, "Extents X", theme, |ui| {
                        ui.add(
                    ...
                    row += 1;
                    modified |= inline_property(ui, row, "Extents Y", theme, |ui| {
                        ui.add(
                ...
                HanabiEmitShape::Box { half_extents } => {
                    ...
                    row += 1;
                    modified |= inline_property(ui, row, "Extents Z", theme, |ui| {
                        ui.add(
...
fn render_velocity_section(
    ...
    collapsible_section(
        ...
        |ui| {
            ...
            row += 1;
            modified |= inline_property(ui, row, "Speed", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.velocity_magnitude)
                        .speed(0.1)
                        .range(0.0..=100.0),
                )
                .changed()
            ...
            row += 1;
            modified |= inline_property(ui, row, "Speed Min", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.velocity_speed_min)
                        .speed(0.1)
                        .range(0.0..=100.0),
                )
                .changed()
            ...
            row += 1;
            modified |= inline_property(ui, row, "Speed Max", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.velocity_speed_max)
                        .speed(0.1)
                        .range(0.0..=100.0),
                )
                .changed()
            ...
            if effect.velocity_mode == VelocityMode::Directional {
                modified |= inline_property(ui, row, "Spread", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut effect.velocity_spread)
                            .speed(0.05)
                ...
                modified |= inline_property(ui, row, "Direction", theme, |ui| {
                    ...
                    changed |= ui
                        .add(egui::DragValue::new(&mut effect.velocity_direction[0]).speed(0.1))
                        .changed();
                    ui.label(RichText::new("Y").size(10.0));
                    changed |= ui
                        .add(egui::DragValue::new(&mut effect.velocity_direction[1]).speed(0.1))
                        .changed();
                    ui.label(RichText::new("Z").size(10.0));
                    changed |= ui
                        .add(egui::DragValue::new(&mut effect.velocity_direction[2]).speed(0.1))
                        .changed();
                    changed
                });
            }
            if effect.velocity_mode == VelocityMode::Tangent {
                modified |= inline_property(ui, row, "Axis", theme, |ui| {
                    ...
                    ui.label(RichText::new("X").size(10.0));
                    changed |= ui
                        .add(egui::DragValue::new(&mut effect.velocity_axis[0]).speed(0.1))
                        .changed();
                    ui.label(RichText::new("Y").size(10.0));
                    changed |= ui
                        .add(egui::DragValue::new(&mut effect.velocity_axis[1]).speed(0.1))
                        .changed();
                    ui.label(RichText::new("Z").size(10.0));
                    changed |= ui
                        .add(egui::DragValue::new(&mut effect.velocity_axis[2]).speed(0.1))
                        .changed();
                    changed
                });
            }
    ...
    collapsible_section(
        ...
        |ui| {
            modified |= inline_property(ui, 0, "Accel", theme, |ui| {
                ...
                ui.label(RichText::new("X").size(10.0));
                changed |= ui
                    .add(egui::DragValue::new(&mut effect.acceleration[0]).speed(0.1))
                    .changed();
                ui.label(RichText::new("Y").size(10.0));
                changed |= ui
                    .add(egui::DragValue::new(&mut effect.acceleration[1]).speed(0.1))
                    .changed();
                ui.label(RichText::new("Z").size(10.0));
                changed |= ui
                    .add(egui::DragValue::new(&mut effect.acceleration[2]).speed(0.1))
                    .changed();
                changed
            });
            ...
            modified |= inline_property(ui, 2, "Drag", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.linear_drag)
                        .speed(0.05)
                        .range(0.0..=10.0),
                )
                .changed()
            });
            modified |= inline_property(ui, 3, "Radial Accel", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.radial_acceleration)
                        .speed(0.1)
                        .range(-100.0..=100.0),
                )
                .changed()
            });
            modified |= inline_property(ui, 4, "Tangent Accel", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.tangent_acceleration)
                        .speed(0.1)
                        .range(-100.0..=100.0),
                )
                .changed()
            ...
            if effect.tangent_acceleration.abs() > 0.001 {
                modified |= inline_property(ui, 5, "Tangent Axis", theme, |ui| {
                    ...
                    ui.label(RichText::new("X").size(10.0));
                    changed |= ui
                        .add(egui::DragValue::new(&mut effect.tangent_accel_axis[0]).speed(0.1))
                        .changed();
                    ui.label(RichText::new("Y").size(10.0));
                    changed |= ui
                        .add(egui::DragValue::new(&mut effect.tangent_accel_axis[1]).speed(0.1))
                        .changed();
                    ui.label(RichText::new("Z").size(10.0));
                    changed |= ui
                        .add(egui::DragValue::new(&mut effect.tangent_accel_axis[2]).speed(0.1))
                        .changed();
                    changed
                });
            }
...
fn render_size_section(
    ...
    collapsible_section(
        ...
        |ui| {
            let mut row = 0;
            modified |= inline_property(ui, row, "Non-Uniform", theme, |ui| {
                ui.checkbox(&mut effect.size_non_uniform, "").changed()
            });
            row += 1;

            if effect.size_non_uniform {
                modified |= inline_property(ui, row, "Start X", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut effect.size_start_x)
                            .speed(0.01)
                            .range(0.001..=10.0),
                    )
                    .changed()
                ...
                row += 1;
                modified |= inline_property(ui, row, "Start Y", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut effect.size_start_y)
                            .speed(0.01)
                            .range(0.001..=10.0),
                    )
                    .changed()
                ...
                row += 1;
                modified |= inline_property(ui, row, "End X", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut effect.size_end_x)
                            .speed(0.01)
                            .range(0.0..=10.0),
                    )
                    .changed()
                ...
                row += 1;
                modified |= inline_property(ui, row, "End Y", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut effect.size_end_y)
                            .speed(0.01)
                            .range(0.0..=10.0),
                    )
                    .changed()
                ...
                modified |= inline_property(ui, row, "Start", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut effect.size_start)
                            .speed(0.01)
                            .range(0.001..=10.0),
                    )
                    .changed()
                ...
                row += 1;
                modified |= inline_property(ui, row, "End", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut effect.size_end)
                            .speed(0.01)
                            .range(0.0..=10.0),
                    )
                    .changed()
            ...
            row += 1;
            modified |= inline_property(ui, row, "Random Min", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.size_start_min)
                        .speed(0.01)
                        .range(0.0..=10.0),
                )
                .changed()
            ...
            row += 1;
            modified |= inline_property(ui, row, "Random Max", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.size_start_max)
                        .speed(0.01)
                        .range(0.0..=10.0),
                )
                .changed()
            ...
            row += 1;
            modified |= inline_property(ui, row, "Screen Space", theme, |ui| {
                ui.checkbox(&mut effect.screen_space_size, "").changed()
            });
            row += 1;
            modified |= inline_property(ui, row, "Roundness", theme, |ui| {
                ui.add(egui::Slider::new(&mut effect.roundness, 0.0..=1.0))
                    .changed()
            });
        },
    );
...
fn render_color_section(
    ...
    collapsible_section(
        ...
        |ui| {
            let mut row = 0;
            modified |= inline_property(ui, row, "Flat Color", theme, |ui| {
            ...
            if effect.use_flat_color {
                ...
                let (rect, _) =
                    ui.allocate_exact_size(Vec2::new(gradient_width, 20.0), Sense::hover());
                ...
                for (i, stop) in effect.color_gradient.iter_mut().enumerate() {
                    modified |=
                        inline_property(ui, row + i, &format!("Stop {}", i + 1), theme, |ui| {
                            let mut changed = false;
                            changed |= ui
                                .add(
                ...
                inline_property(ui, row, "Actions", theme, |ui| {
                    if ui.small_button(format!("{} Add", PLUS)).clicked()
                        && effect.color_gradient.len() < 8
                    {
                        effect.color_gradient.push(GradientStop {
                            position: 0.5,
            ...
            row += 1;
            modified |= inline_property(ui, row, "HDR Color", theme, |ui| {
                ui.checkbox(&mut effect.use_hdr_color, "").changed()
            });
            row += 1;
            if effect.use_hdr_color {
                modified |= inline_property(ui, row, "HDR Intensity", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut effect.hdr_intensity)
                            .speed(0.1)
                            .range(1.0..=100.0),
                    )
                    .changed()
            ...
            modified |= inline_property(ui, row, "Blend Mode", theme, |ui| {
                ...
                egui::ComboBox::from_id_salt("color_blend_mode_editor")
                    ...
                    .show_ui(ui, |ui| {
                        if ui
                            .selectable_label(
                                effect.color_blend_mode == ParticleColorBlendMode::Modulate,
                                "Modulate",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                effect.color_blend_mode == ParticleColorBlendMode::Overwrite,
                                "Overwrite",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                effect.color_blend_mode == ParticleColorBlendMode::Add,
                                "Add",
                            )
                            .clicked()
                        {
    ...
    collapsible_section(
        ...
        |ui| {
            ...
            modified |= inline_property(ui, row, "Alpha Mode", theme, |ui| {
                ...
                egui::ComboBox::from_id_salt("alpha_mode_editor")
                    ...
                    .show_ui(ui, |ui| {
                        if ui
                            .selectable_label(
                                effect.alpha_mode == ParticleAlphaMode::Blend,
                                "Blend",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                effect.alpha_mode == ParticleAlphaMode::Premultiply,
                                "Premultiply",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                effect.alpha_mode == ParticleAlphaMode::Add,
                                "Additive",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                effect.alpha_mode == ParticleAlphaMode::Multiply,
                                "Multiply",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(effect.alpha_mode == ParticleAlphaMode::Mask, "Mask")
                            .clicked()
                        {
                            effect.alpha_mode = ParticleAlphaMode::Mask;
                            changed = true;
                        }
                        if ui
                            .selectable_label(
                                effect.alpha_mode == ParticleAlphaMode::Opaque,
                                "Opaque",
                            )
                            .clicked()
                        {
            ...
            if effect.alpha_mode == ParticleAlphaMode::Mask {
                modified |= inline_property(ui, row, "Mask Threshold", theme, |ui| {
                    ui.add(egui::Slider::new(
                        &mut effect.alpha_mask_threshold,
                        0.0..=1.0,
                    ))
                    .changed()
                });
            ...
            modified |= inline_property(ui, row, "Orient Mode", theme, |ui| {
                ...
                egui::ComboBox::from_id_salt("orient_mode_editor")
                    ...
                    .show_ui(ui, |ui| {
                        if ui
                            .selectable_label(
                                effect.orient_mode == ParticleOrientMode::ParallelCameraDepthPlane,
                                "Camera Plane",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                effect.orient_mode == ParticleOrientMode::FaceCameraPosition,
                                "Face Camera",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                effect.orient_mode == ParticleOrientMode::AlongVelocity,
                                "Along Velocity",
                            )
                            .clicked()
                        {
            ...
            row += 1;
            modified |= inline_property(ui, row, "Rotation Speed", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.rotation_speed)
                        .speed(0.1)
                        .range(-20.0..=20.0)
                        .suffix(" rad/s"),
                )
            ...
            if effect.texture_path.is_some() {
                ...
                modified |= inline_property(ui, row, "Flipbook", theme, |ui| {
                    ...
                    if changed {
                        effect.flipbook = if enabled {
                            Some(FlipbookSettings::default())
                        } else {
                            None
                        };
                    }
                ...
                if let Some(ref mut fb) = effect.flipbook {
                    modified |= inline_property(ui, row, "Grid Cols", theme, |ui| {
                        ui.add(egui::DragValue::new(&mut fb.grid_columns).range(1..=16))
                            .changed()
                    });
                    row += 1;
                    modified |= inline_property(ui, row, "Grid Rows", theme, |ui| {
                        ui.add(egui::DragValue::new(&mut fb.grid_rows).range(1..=16))
                            .changed()
                    });
                    row += 1;
                }
            }
            modified |= inline_property(ui, row, "Layer", theme, |ui| {
                ui.add(egui::DragValue::new(&mut effect.render_layer).range(0..=31))
                    .changed()
            });
        },
    );
    ...
    collapsible_section(
        ...
        |ui| {
            modified |= inline_property(ui, 0, "Space", theme, |ui| {
                let mut changed = false;
                if ui
                    .radio(effect.simulation_space == SimulationSpace::Local, "Local")
                    .clicked()
                {
                ...
                if ui
                    .radio(effect.simulation_space == SimulationSpace::World, "World")
                    .clicked()
                {
            ...
            modified |= inline_property(ui, 2, "Integration", theme, |ui| {
                ...
                egui::ComboBox::from_id_salt("motion_integration_editor")
                    ...
                    .show_ui(ui, |ui| {
                        if ui
                            .selectable_label(
                                effect.motion_integration == MotionIntegrationMode::PostUpdate,
                                "Post-Update",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                effect.motion_integration == MotionIntegrationMode::PreUpdate,
                                "Pre-Update",
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .selectable_label(
                                effect.motion_integration == MotionIntegrationMode::None,
                                "None",
                            )
                            .clicked()
                        {
                ...
                match zone {
                    KillZone::Sphere {
                        ...
                        modified |= inline_property(ui, base_row, "Center", theme, |ui| {
                            ...
                            ui.spacing_mut().item_spacing.x = 2.0;
                            c |= ui
                                .add(egui::DragValue::new(&mut center[0]).speed(0.1))
                                .changed();
                            c |= ui
                                .add(egui::DragValue::new(&mut center[1]).speed(0.1))
                                .changed();
                            c |= ui
                                .add(egui::DragValue::new(&mut center[2]).speed(0.1))
                                .changed();
                            c
                        });
                        modified |= inline_property(ui, base_row + 1, "Radius", theme, |ui| {
                            ui.add(egui::DragValue::new(radius).speed(0.1).range(0.1..=100.0))
                                .changed()
                        });
                        modified |= inline_property(ui, base_row + 2, "Kill Inside", theme, |ui| {
                    ...
                    KillZone::Aabb {
                        ...
                        modified |= inline_property(ui, base_row + 1, "Half Size", theme, |ui| {
                            ...
                            c |= ui
                                .add(
                                    egui::DragValue::new(&mut half_size[0])
                                        .speed(0.1)
                                        .range(0.1..=100.0),
                                )
                                .changed();
                            c |= ui
                                .add(
                                    egui::DragValue::new(&mut half_size[1])
                                        .speed(0.1)
                                        .range(0.1..=100.0),
                                )
                                .changed();
                            c |= ui
                                .add(
                                    egui::DragValue::new(&mut half_size[2])
                                        .speed(0.1)
                                        .range(0.1..=100.0),
                                )
                                .changed();
    ...
    collapsible_section(
        ...
        |ui| {
            ...
            modified |= inline_property(ui, 0, "Enabled", theme, |ui| {
                ...
                let changed = ui.checkbox(&mut enabled, "").changed();
                if changed {
                    effect.conform_to_sphere = if enabled {
                        Some(ConformToSphere::default())
                    } else {
                        None
                    };
                }
            ...
            if let Some(ref mut conform) = effect.conform_to_sphere {
                modified |= inline_property(ui, 1, "Origin", theme, |ui| {
                    ...
                    ui.spacing_mut().item_spacing.x = 2.0;
                    c |= ui
                        .add(egui::DragValue::new(&mut conform.origin[0]).speed(0.1))
                        .changed();
                    c |= ui
                        .add(egui::DragValue::new(&mut conform.origin[1]).speed(0.1))
                        .changed();
                    c |= ui
                        .add(egui::DragValue::new(&mut conform.origin[2]).speed(0.1))
                        .changed();
                    c
                });
                modified |= inline_property(ui, 2, "Radius", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut conform.radius)
                            .speed(0.1)
                            .range(0.1..=100.0),
                    )
                    .changed()
                });
                modified |= inline_property(ui, 3, "Influence Dist", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut conform.influence_dist)
                            .speed(0.1)
                            .range(0.0..=100.0),
                    )
                    .changed()
                });
                modified |= inline_property(ui, 4, "Accel", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut conform.attraction_accel)
                            .speed(0.1)
                            .range(0.0..=100.0),
                    )
                    .changed()
                });
                modified |= inline_property(ui, 5, "Max Speed", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut conform.max_attraction_speed)
                            .speed(0.1)
                            .range(0.0..=100.0),
                    )
                    .changed()
                });
                modified |= inline_property(ui, 6, "Shell Thick.", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut conform.shell_half_thickness)
                            .speed(0.01)
                            .range(0.0..=10.0),
                    )
                    .changed()
                });
                modified |= inline_property(ui, 7, "Sticky Factor", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut conform.sticky_factor)
                            .speed(0.01)
                            .range(0.0..=10.0),
                    )
                    .changed()
    ...
    collapsible_section(
        ...
        |ui| {
            ...
            modified |= inline_property(ui, row, "Frequency", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.noise_frequency)
                        .speed(0.01)
                        .range(0.0..=50.0),
                )
                .changed()
            ...
            modified |= inline_property(ui, row, "Amplitude", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.noise_amplitude)
                        .speed(0.01)
                        .range(0.0..=50.0),
                )
                .changed()
            ...
            modified |= inline_property(ui, row, "Octaves", theme, |ui| {
                ...
                let changed = ui
                    .add(egui::DragValue::new(&mut val).range(1..=8))
                    .changed();
                if changed {
                    effect.noise_octaves = val as u32;
                }
                changed
            });
            row += 1;
            modified |= inline_property(ui, row, "Lacunarity", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.noise_lacunarity)
                        .speed(0.01)
                        .range(1.0..=4.0),
                )
                .changed()
    ...
    collapsible_section(
        ...
        |ui| {
            ...
            modified |= inline_property(ui, 0, "Enabled", theme, |ui| {
                ...
                let changed = ui.checkbox(&mut enabled, "").changed();
                if changed {
                    effect.orbit = if enabled {
                        Some(OrbitSettings::default())
                    } else {
                        None
                    };
                }
            ...
            if let Some(ref mut orbit) = effect.orbit {
                modified |= inline_property(ui, 1, "Center", theme, |ui| {
                    ...
                    ui.spacing_mut().item_spacing.x = 2.0;
                    c |= ui
                        .add(egui::DragValue::new(&mut orbit.center[0]).speed(0.1))
                        .changed();
                    c |= ui
                        .add(egui::DragValue::new(&mut orbit.center[1]).speed(0.1))
                        .changed();
                    c |= ui
                        .add(egui::DragValue::new(&mut orbit.center[2]).speed(0.1))
                        .changed();
                    c
                });
                modified |= inline_property(ui, 2, "Axis", theme, |ui| {
                    ...
                    ui.spacing_mut().item_spacing.x = 2.0;
                    c |= ui
                        .add(egui::DragValue::new(&mut orbit.axis[0]).speed(0.1))
                        .changed();
                    c |= ui
                        .add(egui::DragValue::new(&mut orbit.axis[1]).speed(0.1))
                        .changed();
                    c |= ui
                        .add(egui::DragValue::new(&mut orbit.axis[2]).speed(0.1))
                        .changed();
                    c
                });
                modified |= inline_property(ui, 3, "Speed", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut orbit.speed)
                            .speed(0.01)
                            .range(-20.0..=20.0),
                    )
                    .changed()
                ...
                modified |= inline_property(ui, 4, "Radial Pull", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut orbit.radial_pull)
                            .speed(0.01)
                            .range(0.0..=20.0),
                    )
                    .changed()
                ...
                modified |= inline_property(ui, 5, "Orbit Radius", theme, |ui| {
                    ui.add(
                        egui::DragValue::new(&mut orbit.orbit_radius)
                            .speed(0.1)
                            .range(0.1..=100.0),
                    )
                    .changed()
...
fn render_velocity_limit_section(
    ui: &mut egui::Ui,
    effect: &mut HanabiEffectDefinition,
    theme: &Theme,
) -> bool {
    ...
    collapsible_section(
        ...
        |ui| {
            modified |= inline_property(ui, 0, "Max Speed", theme, |ui| {
                ui.add(
                    egui::DragValue::new(&mut effect.velocity_limit)
                        .speed(0.1)
                        .range(0.0..=1000.0),
                )
                .changed()
...
fn render_variables_section(
    ...
    collapsible_section(
        ...
        |ui| {
            ui.label(
                egui::RichText::new("Exposed to scripts")
                    .size(10.0)
                    .color(theme.text.muted.to_color32()),
            );
                    ...
                    modified |= inline_property(ui, i, name, theme, |ui| {
                        ...
                        match var {
                            ...
                            EffectVariable::Vec3 { value } => {
                                changed |= ui
                                    .add(egui::DragValue::new(&mut value[0]).speed(0.1))
                                    .changed();
                                changed |= ui
                                    .add(egui::DragValue::new(&mut value[1]).speed(0.1))
                                    .changed();
                                changed |= ui
                                    .add(egui::DragValue::new(&mut value[2]).speed(0.1))
                                    .changed();
                            }
                        }
            ...
            inline_property(ui, var_names.len(), "Add", theme, |ui| {
                if ui.small_button(format!("{} Float", PLUS)).clicked() {
                    effect.variables.insert(
                        format!("var_{}", effect.variables.len()),
                ...
                if ui.small_button(format!("{} Color", PLUS)).clicked() {
                    effect.variables.insert(
                        format!("color_{}", effect.variables.len()),
