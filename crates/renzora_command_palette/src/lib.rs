...
mod tests {
    ...
    // Two tests stood here — `server_backed_tabs_wait_for_a_real_query` and
    // `catalog_and_local_tabs_need_no_query` — guarding `min_query_len`, the
    // debounce that kept the remote tabs from firing a search request per
    // keystroke. Every tab they named went with the social removal, and so did
    // the function; all three surviving tabs search locally, so there is no
    // request to debounce and nothing left to assert.

    /// `ALL` is what the tab strip renders from, so a variant missing from it is
    /// a tab the user cannot reach — invisible rather than broken.
    #[test]
