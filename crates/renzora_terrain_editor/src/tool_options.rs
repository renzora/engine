...
pub fn draw_sculpt_options(ui: &mut egui::Ui, world: &World) {
    ...
    shape_picker(ui, settings.brush_shape, cmds, |s, v| {
        if let Some(mut t) = s.get_resource_mut::<TerrainSettings>() {
            t.brush_shape = v;
        }
    });
...
pub fn draw_paint_options(ui: &mut egui::Ui, world: &World) {
    let Some(settings) = world.get_resource::<SurfacePaintSettings>() else {
        return;
    };
    let Some(cmds) = world.get_resource::<EditorCommands>() else {
        return;
    ...
    if ui
        .add(egui::DragValue::new(&mut layer).range(0..=7).speed(0.1))
        .changed()
    {
    ...
    ui.label(RichText::new("Shape").size(11.0).color(muted));
    shape_picker(ui, settings.brush_shape, cmds, |s, v| {
        if let Some(mut t) = s.get_resource_mut::<SurfacePaintSettings>() {
            t.brush_shape = v;
        }
    });
}
