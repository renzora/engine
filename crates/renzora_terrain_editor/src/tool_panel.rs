        ...
        |ui| {
            ...
            if settings.brush_type == TerrainBrushType::Stamp {
                ...
                let stamp_name = stamp.map_or("None".to_string(), |s| {
                    if s.is_loaded() {
                        format!("{} ({}x{})", s.name, s.width, s.height)
                    } else {
                        "None".to_string()
                    }
                });
                ...
                inline_property(ui, row, "Image", theme, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(
                            RichText::new(&stamp_name)
                                .size(11.0)
                                .color(theme.text.secondary.to_color32()),
                        );
                        if ui
                            .button(RichText::new(format!("{FILE_IMAGE} Load")).size(11.0))
                            .clicked()
                        {
                                    ...
                                    match StampBrushData::load_from_png(&data) {
                                        Ok((w, h, pixels)) => {
                                            cmds.push(move |w_: &mut World| {
                                                if let Some(mut stamp) =
                                                    w_.get_resource_mut::<StampBrushData>()
                                                {
                                                    stamp.width = w;
                                                    stamp.height = h;
                                                    stamp.pixels = pixels;
                ...
                inline_property(ui, row, "Rotation", theme, |ui| {
                    ui.add(
                        egui::Slider::new(&mut rotation, 0.0..=360.0)
                            .suffix("°")
                            .show_value(true),
                    )
                });
            ...
            inline_property(ui, row, "Shape", theme, |ui| {
                ui.horizontal(|ui| {
                    for (s, icon) in [
                        ...
                        let active = shape == s;
                        if small_toggle(ui, icon, active, theme).clicked() {
                            shape = s;
                        }
                    }
                })
            ...
            inline_property(ui, row, "Falloff Type", theme, |ui| {
                ui.horizontal(|ui| {
                    for (ft, label) in [
                        ...
                        if small_toggle(ui, label, falloff_type == ft, theme).clicked() {
                            falloff_type = ft;
                        }
                    }
                })
    ...
    if let Some(ps) = world.get_resource::<SurfacePaintSettings>() {
        collapsible_section(
            ...
            |ui| {
                ...
                let spacing = 4.0;
                let btn_size =
                    ((available_width - spacing * (cols as f32 - 1.0)) / cols as f32).max(40.0);
                ui.horizontal(|ui| {
    ...
    if let Some(ps) = world.get_resource::<SurfacePaintSettings>() {
        collapsible_section(
            ...
            |ui| {
                ...
                inline_property(ui, row, "Shape", theme, |ui| {
                    ui.horizontal(|ui| {
                        for (s, icon) in [
                            ...
                            if small_toggle(ui, icon, shape == s, theme).clicked() {
                                shape = s;
                            }
                        }
                    })
                ...
                if radius != ps.brush_radius
                    || strength != ps.brush_strength
                    || falloff != ps.brush_falloff
                    || shape != ps.brush_shape
                {
                    cmds.push(move |w: &mut World| {
                        if let Some(mut s) = w.get_resource_mut::<SurfacePaintSettings>() {
                            s.brush_radius = radius;
...
pub fn render_foliage(ui: &mut egui::Ui, world: &World, theme: &Theme, cmds: &EditorCommands) {
        ...
        |ui| {
            for (i, ft) in config_mut.types.iter().enumerate() {
                ...
                if ui
                    .selectable_label(
                        selected,
                        RichText::new(format!("{}. {}", i + 1, ft.name)).size(12.0),
                    )
                    .clicked()
                {
            ...
            ui.horizontal(|ui| {
                if ui
                    .button(RichText::new(format!("{PLUS} Add")).size(11.0))
                    .clicked()
                {
                ...
                if config_mut.types.len() > 1
                        ...
                        .clicked()
                    {
                        let idx = settings_mut
                            .active_type
                            .min(config_mut.types.len().saturating_sub(1));
    ...
    collapsible_section(
        ...
        |ui| {
            ...
            inline_property(ui, row, "Size", theme, |ui| {
                ui.add(
                    egui::Slider::new(&mut settings_mut.brush_radius, 0.01..=0.5).show_value(true),
                )
            });
            row += 1;
            inline_property(ui, row, "Strength", theme, |ui| {
                ui.add(
                    egui::Slider::new(&mut settings_mut.brush_strength, 0.01..=1.0)
                        .show_value(true),
                )
            });
            row += 1;
            inline_property(ui, row, "Falloff", theme, |ui| {
                ui.add(
                    egui::Slider::new(&mut settings_mut.brush_falloff, 0.0..=1.0).show_value(true),
                )
            });
        },
    );
            ...
            ui.horizontal(|ui| {
                ...
                for col in 0..cols {
                    ...
                    if idx < tools.len() {
                        ...
                        if grid_tool_button(ui, icon, label, active, btn_size, accent, theme)
                            .clicked()
                        {
                            let bt = *brush_type;
                            cmds.push(move |w: &mut World| {
