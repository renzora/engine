...
use egui_phosphor::regular::{
    CROSSHAIR, CUBE, FLOW_ARROW, MAGNIFYING_GLASS_MINUS, MAGNIFYING_GLASS_PLUS, PLUS,
};
...
impl EditorPanel for MaterialGraphPanel {
    ...
    fn ui(&self, ui: &mut egui::Ui, world: &World) {
        ...
        if let Some(path) = asset_path {
            ...
            let mat_ref_path =
                selected_entity.and_then(|e| world.get::<MaterialRef>(e).map(|m| m.0.clone()));
            ...
            let leaving_asset_mode =
                matches!(editor_state.edit_mode, MaterialEditMode::EditingFile { .. });
        ...
        let (graph_modified, selected) = GRAPH_EDITOR_STATE.with(|cell| {
            let mut gs = cell.borrow_mut();
            let m =
                graph_editor::render_graph_editor(ui, &mut graph, &mut gs, &theme, &thumbnail_map);
        ...
        if graph_modified || toolbar_modified || texture_drop_modified {
            ...
            cmds.push(move |world: &mut World| {
                ...
                if let Some(entity) = pending_entity {
                    ...
                    state.edit_mode = MaterialEditMode::Existing {
                        path: asset_path,
                        entity,
                    };
                }
...
fn sync_to_file(world: &mut World, path: String) {
    ...
    warn!(
        "[material_editor] Failed to load asset '{}', starting fresh",
        path
    );
...
fn sync_to_entity(
    ...
    if let Some(path) = mat_ref_path {
        ...
        warn!(
            "[material_editor] Failed to load '{}', starting fresh",
            path
        );
        let name = std::path::Path::new(&path)
            .file_stem()
...
fn render_toolbar(
    ...
    ui.horizontal(|ui| {
        ...
        egui::popup_below_widget(ui, add_id, &add_btn, egui::PopupCloseBehavior::CloseOnClickOutside, |ui| {
            ...
            for &category in &renzora_shader::material::nodes::categories() {
                if category == "Output" {
                    continue;
                }
                let icon = graph_editor::category_icon(category);
                ui.menu_button(format!("{icon} {category}"), |ui| {
                    let defs = renzora_shader::material::nodes::nodes_in_category(category);
                    for def in &defs {
...
fn spawn_texture_nodes(
    ...
    if let Some(tex_node) = graph.get_node_mut(tex_id) {
        tex_node
            .input_values
            .insert("texture".to_string(), PinValue::TexturePath(texture_path));
    }
    true
}
