...
fn render_entity_count_section(ui: &mut egui::Ui, state: &EcsStatsState, theme: &Theme) {
    ...
    ui.horizontal(|ui| {
        ui.label(RichText::new(format!("{}", state.entity_count)).size(32.0).color(theme.text.primary.to_color32()).strong());
        ui.label(RichText::new("entities").size(12.0).color(theme.text.muted.to_color32()));
    });
    ...
    ui.horizontal(|ui| {
        ...
        ui.label(RichText::new(format!("{}", state.archetype_count)).size(11.0).color(theme.text.primary.to_color32()));
        ui.add_space(12.0);
        ui.label(RichText::new("Component Types:").size(11.0).color(theme.text.secondary.to_color32()));
        ui.label(RichText::new(format!("{}", state.component_stats.len())).size(11.0).color(theme.text.primary.to_color32()));
    });
...
fn render_entity_graph(ui: &mut egui::Ui, state: &EcsStatsState, theme: &Theme) {
    let height = 50.0;
    ...
    if points.len() >= 2 {
        ...
        painter.add(egui::Shape::convex_polygon(fill_points, Color32::from_rgba_unmultiplied(100, 180, 220, 30), Stroke::NONE));
...
fn render_archetype_section(ui: &mut egui::Ui, state: &mut EcsStatsState, theme: &Theme) {
    ...
    if header_interact.hovered() { ui.ctx().set_cursor_icon(CursorIcon::PointingHand); }
    if header_interact.clicked() { state.show_archetypes = !state.show_archetypes; }
            ...
            .show(ui, |ui| {
                if state.top_archetypes.is_empty() {
                    ...
                    for (i, archetype) in state.top_archetypes.iter().take(20).enumerate() {
                        ui.horizontal(|ui| {
                            ui.label(RichText::new(format!("{:>5}", archetype.entity_count)).size(10.0).color(theme.text.primary.to_color32()).monospace());
                            let components_str = if archetype.components.len() > 3 {
                                format!("{}, ... +{}", archetype.components.iter().take(3).map(|s| short_name(s)).collect::<Vec<_>>().join(", "), archetype.components.len() - 3)
                            } else {
                                archetype.components.iter().map(|s| short_name(s)).collect::<Vec<_>>().join(", ")
                            };
                            ui.label(RichText::new(components_str).size(10.0).color(theme.text.secondary.to_color32()));
                        });
                        if i < state.top_archetypes.len() - 1 && i < 19 { ui.separator(); }
                    }
                }
            });
    }
...
fn render_component_section(ui: &mut egui::Ui, state: &mut EcsStatsState, theme: &Theme) {
    ...
    let header_interact = header_response.response.interact(egui::Sense::click());
    if header_interact.hovered() { ui.ctx().set_cursor_icon(CursorIcon::PointingHand); }
    if header_interact.clicked() { state.show_components = !state.show_components; }
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                if state.component_stats.is_empty() {
                    ...
                    for comp in state.component_stats.iter().take(30) {
                        ui.horizontal(|ui| {
                            ui.label(RichText::new(format!("{:>6}", comp.instance_count)).size(10.0).color(theme.text.primary.to_color32()).monospace());
                            ui.add_space(8.0);
                            ui.label(RichText::new(short_name(&comp.name)).size(10.0).color(theme.text.secondary.to_color32()));
                        });
                        ui.add_space(2.0);
                    }
                    ...
                    if state.component_stats.len() > 30 {
                        ui.add_space(4.0);
                        ui.label(RichText::new(format!("... and {} more", state.component_stats.len() - 30)).size(10.0).color(theme.text.muted.to_color32()));
                    }
                }
            });
    }
...
fn render_resources_section(ui: &mut egui::Ui, state: &mut EcsStatsState, theme: &Theme) {
    ...
    let header_interact = header_response.response.interact(egui::Sense::click());
    if header_interact.hovered() { ui.ctx().set_cursor_icon(CursorIcon::PointingHand); }
    if header_interact.clicked() { state.show_resources = !state.show_resources; }
            ...
            .show(ui, |ui| {
                if state.resources.is_empty() {
                    ...
                    for resource in state.resources.iter().take(50) {
                        ui.label(RichText::new(short_name(resource)).size(10.0).color(theme.text.secondary.to_color32()));
                    }
                }
            });
    }
