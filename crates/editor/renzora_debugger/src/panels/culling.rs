...
fn render_overview_header(ui: &mut egui::Ui, state: &CullingDebugState, theme: &Theme) {
    ...
    ui.horizontal(|ui| {
        ui.label(RichText::new(format!("{}", visible)).size(28.0).color(theme.text.primary.to_color32()).strong());
        ui.label(RichText::new(format!("/ {} visible", state.total_entities)).size(12.0).color(theme.text.muted.to_color32()));
    });

    if state.total_entities > 0 {
        let pct = (visible as f32 / state.total_entities as f32 * 100.0) as u32;
        ui.label(RichText::new(format!("{}% of mesh entities visible", pct)).size(10.0).color(theme.text.muted.to_color32()));
    }
}
...
fn render_culling_breakdown(ui: &mut egui::Ui, state: &CullingDebugState, theme: &Theme) {
    ...
    egui::Frame::NONE
        ...
        .show(ui, |ui| {
            ...
            render_bar_row(ui, "Visible", visible_after_distance, total, Color32::from_rgb(100, 200, 100), theme);
            render_bar_row(ui, "Frustum Culled", state.frustum_culled, total, Color32::from_rgb(200, 150, 80), theme);
            render_bar_row(ui, "Distance Culled", state.distance_culled, total, Color32::from_rgb(200, 100, 100), theme);

            if state.distance_faded > 0 {
                render_bar_row(ui, "Fading", state.distance_faded, total, Color32::from_rgb(180, 180, 100), theme);
            }
        });
}

fn render_bar_row(ui: &mut egui::Ui, label: &str, count: u32, total: f32, color: Color32, theme: &Theme) {
    ui.horizontal(|ui| {
        ui.label(RichText::new(label).size(10.0).color(theme.text.secondary.to_color32()));
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ui.label(RichText::new(format!("{}", count)).size(10.0).color(theme.text.primary.to_color32()).monospace());
        });
    });

    let fraction = (count as f32 / total).clamp(0.0, 1.0);
    let available_width = ui.available_width();
    let (rect, _) = ui.allocate_exact_size(egui::Vec2::new(available_width, 6.0), egui::Sense::hover());
    ui.painter().rect_filled(rect, 2.0, theme.surfaces.extreme.to_color32());
    let filled = egui::Rect::from_min_size(rect.min, egui::Vec2::new(available_width * fraction, 6.0));
    ...
    egui::Frame::NONE
        ...
        .show(ui, |ui| {
            for (i, (count, label)) in state.distance_buckets.iter().zip(bucket_labels.iter()).enumerate() {
                ui.horizontal(|ui| {
                    ui.label(RichText::new(*label).size(10.0).color(theme.text.secondary.to_color32()).monospace());
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ui.label(RichText::new(format!("{}", count)).size(10.0).color(theme.text.primary.to_color32()).monospace());
                    });
                });
                ...
                let available_width = ui.available_width();
                let (rect, _) = ui.allocate_exact_size(egui::Vec2::new(available_width, 4.0), egui::Sense::hover());
                ui.painter().rect_filled(rect, 2.0, theme.surfaces.extreme.to_color32());

                let ci = (i as f32 / 4.0 * 0.6 + 0.4).min(1.0);
                let bar_color = Color32::from_rgb((80.0 + 120.0 * ci) as u8, (160.0 - 60.0 * ci) as u8, 200);
                let filled = egui::Rect::from_min_size(rect.min, egui::Vec2::new(available_width * fraction, 4.0));
                ui.painter().rect_filled(filled, 2.0, bar_color);

                if i < 4 { ui.add_space(2.0); }
            }
        });
}
...
fn render_settings(ui: &mut egui::Ui, state: &mut CullingDebugState, theme: &Theme) {
    ...
    egui::Frame::NONE
        ...
        .show(ui, |ui| {
            ...
            ui.add_space(8.0);
            ui.horizontal(|ui| {
                ui.label(RichText::new("Max Distance:").size(10.0).color(theme.text.secondary.to_color32()));
                ui.add(egui::Slider::new(&mut state.max_distance, 10.0..=2000.0).suffix("m"));
            });
            ui.add_space(4.0);
            ui.horizontal(|ui| {
                ui.label(RichText::new("Fade Start:").size(10.0).color(theme.text.secondary.to_color32()));
