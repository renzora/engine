//! Systems that apply viewport header render toggles and visualization modes.
...
/// Stores original material properties so they can be restored.
...
#[derive(Resource, Default)]
pub struct LastRenderState {
    toggles: Option<RenderToggles>,
    visualization: Option<VisualizationMode>,
}

/// Update rendering based on render toggles and visualization mode.
pub fn update_render_toggles(
    ...
    let new_materials_added = material_events
        .read()
    ...
    for (id, material) in materials.iter_mut() {
        ...
        match current_viz {
            ...
            VisualizationMode::Roughness => {
                let r = original.as_ref().map_or(material.perceptual_roughness, |o| o.perceptual_roughness);
                material.base_color = Color::srgb(r, r, r);
                material.base_color_texture = None;
                material.unlit = true;
            }
            VisualizationMode::Metallic => {
                let m = original.as_ref().map_or(material.metallic, |o| o.metallic);
                material.base_color = Color::srgb(m, m, m);
                material.base_color_texture = None;
                material.unlit = true;
            }
        ...
        // Apply toggles when no viz mode
        ...
        // Wireframe + no textures = transparent
...
/// Update shadow settings on all lights.
