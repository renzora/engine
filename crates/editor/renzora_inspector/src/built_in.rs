...
fn animator_ui(
    ...
    let Some(animator) = world.get::<AnimatorComponent>(entity) else { return };
    let animator = animator.clone();
    let payload = world.get_resource::<AssetDragPayload>();

    // Default Clip
    inline_property(ui, 0, "Default Clip", theme, |ui| {
        let current = animator.default_clip.as_deref().unwrap_or("(none)");
        if animator.clips.is_empty() {
            ui.label(egui::RichText::new("(no clips)").size(11.0).color(theme.text.muted.to_color32()));
        } else {
            egui::ComboBox::from_id_salt("animator_default_clip")
                .selected_text(current)
                .width(ui.available_width())
                .show_ui(ui, |ui| {
                    if ui.selectable_label(animator.default_clip.is_none(), "(none)").clicked() {
                        cmds.push(move |w: &mut World| {
                    ...
                    for slot in &animator.clips {
                        ...
                        let selected = animator.default_clip.as_deref() == Some(&name);
    ...
    // Blend Duration
    inline_property(ui, 1, "Blend Duration", theme, |ui| {
        let mut v = animator.blend_duration;
        if ui.add(egui::DragValue::new(&mut v).speed(0.01).range(0.0..=5.0).suffix("s")).changed() {
            cmds.push(move |w: &mut World| {
                if let Some(mut a) = w.get_mut::<AnimatorComponent>(entity) { a.blend_duration = v; }
            });
        }
    });

    // State Machine path
    inline_property(ui, 0, "State Machine", theme, |ui| {
        let current_str = animator.state_machine.as_deref();
        let drop = asset_drop_target(
            ui,
            ui.id().with("animator_sm_path"),
            current_str,
            &["animsm"],
            "Drop .animsm file",
            theme,
    ...
    // ── Clip slots ──────────────────────────────────────────────────────────
    ui.add_space(6.0);
    ui.label(egui::RichText::new("Animation Clips").size(11.0).color(theme.text.secondary.to_color32()));
    ui.add_space(2.0);

    let mut remove_index: Option<usize> = None;

    for (i, slot) in animator.clips.iter().enumerate() {
        let row_base = i * 4;

        // Name
        inline_property(ui, row_base, "Name", theme, |ui| {
            let mut name = slot.name.clone();
            if ui.add(egui::TextEdit::singleline(&mut name).desired_width(ui.available_width())).changed() {
                cmds.push(move |w: &mut World| {
                    if let Some(mut a) = w.get_mut::<AnimatorComponent>(entity) {
                        if let Some(s) = a.clips.get_mut(i) { s.name = name; }
                    }
                });
            }
        });

        // Path (drop target for .anim files)
        inline_property(ui, row_base + 1, "Clip", theme, |ui| {
            let current_str = if slot.path.is_empty() { None } else { Some(slot.path.as_str()) };
            let drop = asset_drop_target(
                ui,
                ui.id().with(("anim_clip_path", i)),
                current_str,
                &["anim", "gltf", "glb"],
                "Drop animation file",
                theme,
            ...
            if let Some(path) = drop.dropped_path {
                ...
                cmds.push(move |w: &mut World| {
                    if let Some(mut a) = w.get_mut::<AnimatorComponent>(entity) {
                        if let Some(s) = a.clips.get_mut(i) { s.path = rel_str; }
                    }
                });
            }
            if drop.cleared {
                cmds.push(move |w: &mut World| {
                    if let Some(mut a) = w.get_mut::<AnimatorComponent>(entity) {
                        if let Some(s) = a.clips.get_mut(i) { s.path.clear(); }
                    }
                });
            }
        });

        // Looping + Speed on one row
        inline_property(ui, row_base + 2, "Looping", theme, |ui| {
            let id = ui.id().with(("anim_loop", i));
            if toggle_switch(ui, id, slot.looping) {
                let val = !slot.looping;
                cmds.push(move |w: &mut World| {
                    if let Some(mut a) = w.get_mut::<AnimatorComponent>(entity) {
                        if let Some(s) = a.clips.get_mut(i) { s.looping = val; }
                    }
                });
            }
        });

        inline_property(ui, row_base + 3, "Speed", theme, |ui| {
            let mut v = slot.speed;
            if ui.add(egui::DragValue::new(&mut v).speed(0.01).range(0.01..=10.0)).changed() {
                cmds.push(move |w: &mut World| {
                    if let Some(mut a) = w.get_mut::<AnimatorComponent>(entity) {
                        if let Some(s) = a.clips.get_mut(i) { s.speed = v; }
                    }
                });
            }
        });
        ...
        // Remove clip button
        inline_property(ui, row_base, "", theme, |ui| {
    ...
    if let Some(idx) = remove_index {
        cmds.push(move |w: &mut World| {
            if let Some(mut a) = w.get_mut::<AnimatorComponent>(entity) {
                if idx < a.clips.len() { a.clips.remove(idx); }
            }
        });
    }
    ...
    ui.horizontal(|ui| {
        if ui.add(egui::Button::new(
            egui::RichText::new(format!("{} Add Clip", regular::PLUS)).size(11.0),
        )).clicked() {
            cmds.push(move |w: &mut World| {
                if let Some(mut a) = w.get_mut::<AnimatorComponent>(entity) {
                    a.clips.push(AnimClipSlot::new("new_clip", ""));
                }
            });
        }
    });
