            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                if connected_count == 0 {
                    ui.vertical_centered(|ui| {
                        ui.add_space(40.0);
                        ui.label(
                            RichText::new("No gamepad detected")
                                .size(14.0)
                                .color(Color32::from_gray(120)),
                        );
                        ui.add_space(8.0);
                        ui.label(
                            RichText::new("Connect a controller to see input")
                                .size(12.0)
...
fn render_gamepad(ui: &mut egui::Ui, index: usize, gamepad: &GamepadInfo) {
    ...
    ui.horizontal(|ui| {
        ui.vertical(|ui| {
            ...
            ui.label(
                RichText::new(format!("{:.2}", gamepad.left_trigger))
                    .size(10.0)
                    .color(Color32::from_gray(120)),
            );
        });
        ...
        ui.vertical(|ui| {
            ...
            ui.label(
                RichText::new(format!("{:.2}", gamepad.right_trigger))
                    .size(10.0)
                    .color(Color32::from_gray(120)),
            );
        });
    ...
    if !gamepad.raw_axes.is_empty() {
        ...
        for (name, value) in &gamepad.raw_axes {
            ui.horizontal(|ui| {
                ui.label(
                    RichText::new(format!("{}: {:.3}", name, value))
                        .size(10.0)
                        .color(Color32::from_gray(120)),
                );
            });
