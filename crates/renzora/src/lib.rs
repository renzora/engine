...
// ── Plugin ABI hash ──────────────────────────────────────────────────────
// `RENZORA_ABI_HASH` / `RENZORA_ABI_INPUTS` / `RENZORA_ABI_INPUTS_CSTR`,
// generated by `build.rs` from exactly three inputs: bevy version, rust
// toolchain, and the curated bevy feature set. The `add!` macro returns
// `RENZORA_ABI_HASH` from `plugin_bevy_hash()`, and the dynamic plugin loader
// compares against the same const — so the value crosses the dlopen boundary as
// one shared definition. See the `abi_hash` crate for why this replaced the old
// `TypeId::of::<World>()` tag (it drifted on dependency source / lockfile /
// profile / target with no real ABI change).
