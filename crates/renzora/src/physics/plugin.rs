//! The simulation: the plugin, its systems, and the avian backends behind them.
//!
//! This is here rather than in `renzora_physics` for one reason: a native
//! plugin links `bevy`, `renzora` and `renzora_ember` and nothing else, so
//! anything it must be able to *name* has to be reachable from one of the
//! three. That used to mean the data types alone, with a function-pointer
//! bridge standing in for the rest. It now means the whole subsystem, so a
//! plugin can take a real `SpatialQuery`, spawn a real `RigidBody`, and read a
//! real contact.
//!
//! `renzora_physics` still exists and still owns the one piece that cannot
//! come: its `ScriptExtension` is defined over `renzora_scripting`, which
//! depends on this crate, and moving it would close a dependency cycle.

use bevy::prelude::*;

use crate::physics::data::*;
use crate::physics::read_state::PhysicsReadState;
use crate::PlayModeState;
...
/// When `active`, the editor enters "edit collider" mode for the selected
/// entity: the transform gizmo is hidden and collider resize/move handles take
/// over.
///
/// Read by `renzora_gizmo` to drive those handles, and *initialised* by
/// `renzora_physics_editor::PhysicsEditorPlugin` rather than here — the gizmo
/// crate treats it as optional so the lean runtime, which has no editor, simply
/// never inserts it. Re-exported from `renzora_physics::ColliderEditMode`,
/// which is the path everything already uses.
...
// `CollisionShapeData`, …). The whole simulation backend (`crate::physics::backend::avian`,
...
impl Plugin for PhysicsPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.register_type::<PhysicsBodyData>()
            ...
            .register_type::<crate::physics::read_state::CollisionReadState>();

        #[cfg(feature = "avian3d")]
        app.add_plugins(crate::physics::backend::avian::AvianBackendPlugin { start_paused });

        #[cfg(feature = "avian2d")]
        app.add_plugins(crate::physics::backend::avian_2d::Avian2dBackendPlugin { start_paused });

        app.add_systems(Update, (auto_init_physics, sync_physics_data));
        app.add_systems(
            Update,
            (
                crate::physics::auto_fit::mark_new_collision_shapes,
                crate::physics::auto_fit::auto_fit_collision_shapes,
            )
                .chain(),
        );
        ...
        // The C-ABI surface for standalone plugins is NOT installed here. It
        // stays in `renzora_physics::plugin_bridge` and is installed by that
        // crate's wrapper plugin, because it needs `renzora_plugin/host` —
        // which pulls `libloading` and a file watcher, neither of which belongs
        // in the crate every other crate depends on. It is also the wrong
        // boundary for this move: a native plugin has the real avian and never
        // touches it.

        // The backend-agnostic query bridge, for callers that want to survive a
        // change of backend or do not know the dimension ahead of time.
        #[cfg(feature = "avian3d")]
        crate::physics::spatial_impl::install(app);

        // Per-entity read-state mirror + script extension.
        app.add_systems(Update, crate::physics::read_state::auto_init_physics_read_state);
        app.add_systems(Update, crate::physics::read_state::auto_init_collision_read_state);
        #[cfg(feature = "avian3d")]
        app.add_systems(Update, crate::physics::read_state::update_physics_read_state);
        #[cfg(feature = "avian3d")]
        app.add_systems(Update, crate::physics::read_state::update_collision_read_state);
        #[cfg(feature = "avian2d")]
        app.add_systems(Update, crate::physics::read_state::update_physics_read_state_2d);
        #[cfg(feature = "avian2d")]
        app.add_systems(Update, crate::physics::read_state::update_collision_read_state_2d);
...
fn compute_kinematic_slides(
    ...
    for slide in std::mem::take(&mut queue.0) {
        ...
        let result = crate::physics::backend::avian_character::shape_cast_slide(
            &spatial_query,
            collider,
            shape_origin,
            transform.rotation,
...
pub fn spawn_physics_body(commands: &mut Commands, entity: Entity, body_data: &PhysicsBodyData) {
    #[cfg(feature = "avian3d")]
    crate::physics::backend::avian::spawn_physics_body(commands, entity, body_data);
}
...
pub fn spawn_collision_shape(
    ...
    #[cfg(feature = "avian3d")]
    crate::physics::backend::avian::spawn_collision_shape(commands, entity, shape_data);
}
...
pub fn despawn_physics_components(commands: &mut Commands, entity: Entity) {
    #[cfg(feature = "avian3d")]
    crate::physics::backend::avian::despawn_physics_components(commands, entity);
    #[cfg(feature = "avian2d")]
    crate::physics::backend::avian_2d::despawn_physics_components(commands, entity);
...
fn entity_is_2d(
    entity: Entity,
    markers_2d: &Query<(), Or<(With<Physics2d>, With<Sprite>, With<crate::Node2d>)>>,
...
fn auto_init_physics(
    ...
    for (entity, body, shape, name) in &new_bodies {
        ...
        if is_2d {
            ...
            {
                if let Some(b) = body {
                    crate::physics::backend::avian_2d::spawn_physics_body(&mut commands, entity, b);
                ...
                if let Some(s) = shape {
                    crate::physics::backend::avian_2d::spawn_collision_shape(&mut commands, entity, s);
                }
...
fn sync_physics_data(
    ...
    for (entity, body_data, is_2d) in &changed_bodies {
        if is_2d {
            #[cfg(feature = "avian2d")]
            crate::physics::backend::avian_2d::spawn_physics_body(&mut commands, entity, body_data);
        } else {
    ...
    for (entity, shape_data, is_2d) in &changed_shapes {
        if is_2d {
            #[cfg(feature = "avian2d")]
            crate::physics::backend::avian_2d::spawn_collision_shape(&mut commands, entity, shape_data);
        } else {
...
fn on_pause_physics(_trigger: On<crate::PausePhysics>, mut commands: Commands) {
    commands.queue(|world: &mut World| pause(world));
}

/// Observer: unpause physics when the editor sends `UnpausePhysics`.
fn on_unpause_physics(_trigger: On<crate::UnpausePhysics>, mut commands: Commands) {
...
pub fn unpause(world: &mut World) {
    ...
    crate::console_log::console_info("Physics", "Physics simulation unpaused");
    #[cfg(feature = "avian3d")]
    {
...
pub fn pause(world: &mut World) {
    ...
    crate::console_log::console_info("Physics", "Physics simulation paused");
    #[cfg(feature = "avian3d")]
    {
        use avian3d::schedule::PhysicsTime;
        if let Some(mut time) = world.get_resource_mut::<Time<avian3d::prelude::Physics>>() {
