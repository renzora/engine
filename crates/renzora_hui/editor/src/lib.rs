//! Editor-only half of `renzora_hui` — the HTML-template editor integration
...
//! `MarkupImage`) and call `renzora_hui::writeback::write_attr_to_markup`.
...
/// Editor-scope companion to `renzora_hui::HuiPlugin`. Reproduces the
