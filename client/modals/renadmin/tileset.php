<div data-window='renadmin_tileset_manager' class='window window_bg' style='width: 900px; height: 550px; background: #242d39;'>
  ...
  <div class='relative container text-white window_body p-2' style="display: flex; flex-direction: column; height: 100%;">
    <div style="margin-bottom: 10px;">
      ...
      <select id="editTilesetSelect" class="bg-gray-800 text-white py-2 px-4 mr-1 rounded" onchange="renadmin_tileset_manager.loadTilesetObjects()">
        <?php
        foreach ($tilesets as $tileset) {
            if (is_file("$tilesetDir/$tileset")) {
                echo "<option value='$tileset'>$tileset</option>";
    ...
    <div id="gridView" style="display: flex; height: 100%;">
      <div id="objectGrid" class="grid grid-cols-12 overflow-y-auto w-full"></div>
    </div>
    <div id="propertiesView" style="display: none; height: 100%; width: 100%; padding-left: 10px; overflow-y: scroll;">
  ...
  start: function() {
    ...
    document.getElementById('addToTilesetBtn').addEventListener('click', async function() {
      ...
      if (!objectName) {
        ...
        console.log('Please enter a name for the tileset.');
        return;
      }

      try {
        const response = await fetch(assets.noCache('assets/json/objectData.json'));
        ...
        console.log('Highest i value:', renadmin_tileset_manager.highestIValue);

        // Load the tileset image
        const tilesetImageResponse = await fetch(assets.noCache(`/assets/img/tiles/${tilesetName}.png`));
        ...
        img.onload = function() {
          ...
          const totalTiles = renadmin_tileset_manager.highestIValue + 1 + renadmin_tileset_manager.selectedTiles.length; // total tiles considering the highest index and new tiles
          ...
          // Log the selected tiles and new tiles details
          console.log('Selected tiles:', renadmin_tileset_manager.selectedTiles);
          ...
          canvas.toBlob(async function(blob) {
            ...
            if (uploadResult.success) {
              ...
              ui.ajax({
                outputType: 'json',
                method: 'POST',
                url: 'modals/renadmin/ajax/add_objects.php',
                data: JSON.stringify(objectData),
                success: function(data) {
                  console.log('Object data save response:', data);
                  if (data.success) {
        ...
        console.log('Error updating objectData.json:', error);
      }
    });
      ...
      try {
        ...
        for (const key in groupedObjects) {
          ...
          group.forEach(obj => {
            ...
            const img = new Image();
            img.src = assets.noCache(`/assets/img/tiles/${tilesetName}.png`);
            img.onload = function() {
    ...
    renadmin_tileset_manager.showPropertiesForObject = function(name, objectGroup, tilesetName) {
