sprite = {
  ...
  // 1) We add an `init` function at the top level so we can register key events via input.assign()
  init: function() {
  ...
  //    Decide how to route the key to the correct sprite(s).
  handleKeyDownGlobal: function(e) {
      // For example, let's say we only want the player's sprite to respond:
      const playerSprite = game.sprites[game.playerid];
      if (!playerSprite) return;

      // Find which direction is associated with e.key (from input.keys)
      const dir = input.keys[e.key];
      if (dir) {
          // Use the existing sprite method
  ...
  SpritePrototype: {
      ...
      // 4) You already have these add/remove direction methods. 
      //    We just call them from handleKeyDownGlobal/handleKeyUpGlobal above.
      ...
      walkToClickedTile: function(tileX, tileY) {
          ...
          if(plugin.exists('debug')) debug.tracker('sprite.walkToClickedTile');
      },
      ...
      calculatePath: function(startX, startY, endX, endY) {
          if(plugin.exists('collision')) {
              ...
              if(plugin.exists('debug')) debug.tracker('sprite.calculatePath');
      ...
      moveAlongPath: function() {
          ...
          if(plugin.exists('debug')) debug.tracker('sprite.moveAlongPath');
      },
      ...
      update: function() {
          // 1. If we’re pathfinding, let moveAlongPath handle the movement + collision
          if (this.isMovingToTarget) {
              this.moveAlongPath();
              // We still animate, so no early return:
              this.animate();
              // Optionally sync lighting
          ...
          // 3. If there's no movement at all => idle
          if (dx === 0 && dy === 0) {
              this.moving = false;
              this.stopping = true;
          ...
          // 4. If we do have movement => check collision
