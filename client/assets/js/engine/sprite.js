sprite = {
    ...
    SpritePrototype: {
      ...
      update: function() {
        ...
        // 3. If there's no movement at all, do idle animation -- but keep animating each frame
        if (dx === 0 && dy === 0) {
          // Mark we are not moving
          this.moving = false;
          this.stopping = true;
      
          // Switch to idle unless there's an override
          if (!this.overrideAnimation) {
            this.changeAnimation('idle');
          }
      
          // We still call animate so that idle frames advance
          this.animate();
        ...
        // 4. If we do have movement => single collision check
        ...
        if (plugin.exists('collision')) {
          ...
          if (!collisionResult.collisionDetected) {
            ...
            // partial "slide" if your collision plugin supports it
            this.x += collisionResult.slideVector.x;
            this.y += collisionResult.slideVector.y;
          }
          // else => collision, no movement
        } else {
          // If no collision plugin, move freely
          this.x = newX;
          this.y = newY;
        }
        ...
        // Mark that we are actually moving
        this.moving = true;
        this.stopping = false;
      
        // 6. Change movement animation if there's no override
        if (!this.overrideAnimation) {
          if (this.speed < 50) {
            this.changeAnimation('speed_1');
