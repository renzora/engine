...
const ObjectProperties = ({ objectId }) => {
  const { objectProperties } = useSnapshot(globalStore.editor);
  const { updateObjectProperty, bindNodeToProperty, unbindNodeFromProperty } = actions.editor;
  
  const objectProps = objectProperties.objects[objectId];
  ...
  const handleDrop = (e, propertyPath) => {
    ...
    try {
      ...
      if (data.type === 'asset' && data.fileType === 'texture') {
        updateObjectProperty(objectId, propertyPath, data.path);
          ...
          <div key={axis} className="relative">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                updateObjectProperty(objectId, propertyPath, newValue);
              }}
    ...
    <div className="mb-3">
      ...
      <div className="flex items-center gap-1">
        <input
          type="color"
          value={value || '#ffffff'}
          onChange={(e) => updateObjectProperty(objectId, propertyPath, e.target.value)}
  ...
  const renderSliderInput = (label, value, propertyPath, min = 0, max = 1, step = 0.01) => (
    <div className="mb-3">
      ...
      <input
        ...
        step={step}
        value={value || 0}
        onChange={(e) => updateObjectProperty(objectId, propertyPath, parseFloat(e.target.value))}
      ...
      >
        {value ? (
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => updateObjectProperty(objectId, propertyPath, null)}
          ...
          <div className="space-y-2">
            <label className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => updateObjectProperty(objectId, 'rendering.castShadows', e.target.checked)}
                className="rounded"
              />
            ...
            <label className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => updateObjectProperty(objectId, 'rendering.receiveShadows', e.target.checked)}
                className="rounded"
              />
            ...
            <div className="mb-4">
              ...
              <label className="flex items-center space-x-2 mb-2">
                <input
                  ...
                  onChange={(e) => updateObjectProperty(objectId, 'components.scripting.enabled', e.target.checked)}
                  className="rounded"
                />
              ...
              <div
                ...
                onDragOver={handleDragOver}
              >
                {objectProps.components.scripting.scriptFile ? (
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-300 truncate">
                      {objectProps.components.scripting.scriptFile.split('/').pop()}
                    </span>
                    <button
                      onClick={() => updateObjectProperty(objectId, 'components.scripting.scriptFile', null)}
                      className="text-red-400 hover:text-red-300 ml-2 text-sm"
                    >
                      ×
                    </button>
            ...
            <div className="mb-4">
              ...
              <label className="flex items-center space-x-2 mb-2">
                <input
                  ...
                  onChange={(e) => updateObjectProperty(objectId, 'components.physics.enabled', e.target.checked)}
                  className="rounded"
                />
              ...
              <select
                ...
                onChange={(e) => updateObjectProperty(objectId, 'components.physics.type', e.target.value)}
